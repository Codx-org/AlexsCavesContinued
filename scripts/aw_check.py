#!/usr/bin/env python3
"""Validate the Fabric access-widener TEMPLATE against the UNPATCHED Mojmap Minecraft jars.
...
the members this mod widens move between MC versions — `AbstractArrow#startFalling` changed
package at 1.21.11, `SurfaceRules$Context#updateY` lost two parameters at 26.2, and a whole
block of entries exists only because Forge and NeoForge ship a PATCHED jar in which the member
is already public. So one widener cannot serve 1.20.1 -> 26.2 and every candidate entry has to
be checked against the real bytecode, per node, before a node is added.

Running `:<node>:build` would answer the same question, but only one node at a time and only
after a full loom setup; this answers it for every cached version in a couple of seconds, and
prints the descriptor it DID find so a moved member can be re-gated rather than merely deleted.

It understands the `#?` predicate directives of the generator's source template
(accesswidener/alexscaves.accesswidener, expanded per node by build-logic/AccessWidener.kt),
so pointing it at the TEMPLATE checks each version against exactly the entries that version
would be given. Pointed at a plain widener it behaves the same, since a file with no
directives gates nothing.

Usage:
    scripts/aw_check.py [<accesswidener-file>] [<mc-version> ...]
    scripts/aw_check.py --list-jars <mc-version>

With no arguments it checks accesswidener/alexscaves.accesswidener against ALL_VERSIONS below,
which is the 22 planned Fabric nodes.

It reads the Mojmap jars loom has already downloaded under
~/.gradle/caches/fabric-loom/minecraftMaven, deliberately skipping every loader-patched and
every intermediary/yarn-namespace artifact — in those, everything this script exists to detect
is either already public or spelled under different names.
"""
...
# The artifacts that together hold the whole game, in both flavours this tree produces:
# the remapped ("named" = Mojmap) jars every node below 26 builds against, and the "-deobf"
# jars the 26.x loom-no-remap line uses, which ship Mojang's own names already. The
# loader-patched jars live under sibling artifact names (forge-<mc>-<build>-minecraft-merged),
# so an explicit whitelist is what keeps them out.
ARTIFACTS = (
    "minecraft-common",
    "minecraft-clientonly",
    "minecraft-merged",
    "minecraft-common-deobf",
    "minecraft-clientonly-deobf",
    "minecraft-merged-deobf",
)

# The 22 planned Fabric nodes, in build order. Kept here so a bare run checks the whole matrix.
ALL_VERSIONS = (
...
def vanilla_jars(mc: str) -> list[Path]:
    """Mojmap-namespace vanilla jars for `mc`, excluding every loader-patched variant."""
    found: list[Path] = []
    for artifact in ARTIFACTS:
        ...
        for version_dir in base.iterdir():
            ...
            # A "-deobf" dir is named for the version alone; a remapped one carries the
            # mappings id after it. "1.21-" must not swallow "1.21.1-".
            if name != mc and not name.startswith(f"{mc}-"):
                continue
            # "-forge-"/"-neoforge-" is the loader's PATCHED jar; "yarn"/"intermediary" is the
            # wrong namespace. Either would hide exactly the differences this script looks for.
            if "-forge-" in name or "-neoforge-" in name:
                continue
            if "yarn" in name or "intermediary" in name or "intermediary" in artifact:
                continue
...
def check(aw_path: Path, mc: str) -> int:
    ...
    if not jars:
        print(f"{mc}: no cached vanilla Mojmap jars — build a node for it first, skipping")
        return 0
    classpath = ":".join(str(j) for j in jars)
    index = class_entries(jars)

    problems = 0
    active = active_entries(aw_path, mc)
    # Only the entries THIS version would be given — a template's other arms name members that
    # are supposed to be absent here, and reporting them would drown the real findings.
    for lineno, line in active:
        parts = line.split()
        if len(parts) < 3:
            print(f"{mc}: {aw_path.name}:{lineno}: MALFORMED    {line!r}")
            problems += 1
            continue
        ...
        if kind == "class":
            continue
        if len(parts) < 5:
            print(f"{mc}: {aw_path.name}:{lineno}: NO DESCRIPTOR {line!r}")
            problems += 1
            continue
    ...
    print(f"{mc}: {aw_path.name} entries={len(active)} problems={problems}")
    return problems


DEFAULT_AW = Path(__file__).resolve().parent.parent / "accesswidener/alexscaves.accesswidener"
...
def main() -> int:
    ...
    if args and args[0] == "--list-jars":
        for jar in vanilla_jars(args[1]):
            print(jar)
        return 0
    aw = Path(args[0]) if args and not args[0][0].isdigit() else DEFAULT_AW
    versions = [a for a in args if a and a[0].isdigit()] or list(ALL_VERSIONS)
    if not aw.is_file():
        print(f"no such access widener: {aw}")
        return 2
    return 1 if sum(check(aw, mc) for mc in versions) else 0
