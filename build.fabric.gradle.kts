...
// template at accesswidener/ (repo root — NOT under src/, which Stonecutter would preprocess and
// whose first line has to be the header). ⚠️ Unlike an AT entry, an AW entry naming an absent member is
...
dependencies {
	...
	// CodxLib — per-node jar from mavenLocal (codx:codxlib:<ver>-fabric+<mc>). On classic (<26)
	// Fabric the published jar is remapped to the INTERMEDIARY namespace, so consume it with
	// modImplementation → loom remaps it to this node's Mojmap names (plain implementation would
	// leak class_1792 etc.).
	modImplementation("codx:codxlib:${prop("deps.codxlib")}-fabric+${prop("deps.minecraft")}")

	// JSR-305 (@Nullable / @Nonnull). Upstream Alex's Caves annotates ~120 files with it and gets it
