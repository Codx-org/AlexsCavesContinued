...
5. **`logoFile` comes from `mod.fabric.icon`** (`assets/alexscaves/textures/misc/mod_logo.png`),
   because this tree kept the logo where the upstream Forge manifest pointed rather than at
   Fabric's `assets/<modid>/icon.png` convention. One property feeds both manifests.
