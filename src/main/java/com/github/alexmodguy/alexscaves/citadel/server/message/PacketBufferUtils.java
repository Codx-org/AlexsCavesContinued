...
public class PacketBufferUtils {
    ...
    public static void writeItemStack(ByteBuf to, ItemStack stack) {
        // 1.20.5 moved item encoding off FriendlyByteBuf and onto a StreamCodec that needs registry
        // access to resolve component types, which only a RegistryFriendlyByteBuf carries. Every
        // caller hands this the play-protocol buffer it was given, and Forge types that protocol as
        // RegistryFriendlyByteBuf, so the cast holds — wrapping a bare ByteBuf the way the older arm
        // does would produce a buffer with no registries and fail on the first component.
        //? if >=1.20.5 {
        /*ItemStack.OPTIONAL_STREAM_CODEC.encode((net.minecraft.network.RegistryFriendlyByteBuf) to, stack);
        *///?} else {
    ...
    public static ItemStack readItemStack(ByteBuf from) {
        // See writeItemStack for why the newer arm casts rather than wraps.
        //? if >=1.20.5 {
        /*return ItemStack.OPTIONAL_STREAM_CODEC.decode((net.minecraft.network.RegistryFriendlyByteBuf) from);
        *///?} else {
