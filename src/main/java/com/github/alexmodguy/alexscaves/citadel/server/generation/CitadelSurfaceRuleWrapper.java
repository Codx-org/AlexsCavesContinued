...
public record CitadelSurfaceRuleWrapper(SurfaceRules.RuleSource vanillaRules,
    ...
    // 26.2 unwrapped SurfaceRules' two dispatch interfaces: RuleSource#codec() and
    // ConditionSource#codec() return the MapCodec directly rather than the KeyDispatchDataCodec
    // holder. The holder class itself survives, so only the override moves.
    //? if >=26.2 {
    /*@Override
    public com.mojang.serialization.MapCodec<? extends SurfaceRules.RuleSource> codec() {
        return CODEC.codec();
    }
    *///?} else {
    @Override
