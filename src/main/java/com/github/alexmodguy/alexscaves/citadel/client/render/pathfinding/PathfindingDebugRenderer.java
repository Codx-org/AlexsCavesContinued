...
public class PathfindingDebugRenderer {
    // 1.20.3 made RenderBuffers size its chunk-geometry buffer pool up front, and deleted the no-arg
    // constructor when it did. This debug renderer only ever uses bufferSource(), so it asks for the
    // smallest pool there is. javap says 1.20.2 still has the no-arg form, so the boundary is 1.20.3.
    //? if >=1.20.3
    /*public static final RenderBuffers renderBuffers = new RenderBuffers(1);*/
    //? if <1.20.3
    ...
    private static void renderDebugText(@NotNull final MNode n, final WorldEventContext ctx) {
        ...
        ctx.poseStack.mulPose(com.github.alexmodguy.alexscaves.client.ACClientCompat.cameraOrientation());
        ...
        com.github.alexmodguy.alexscaves.client.ACClientCompat.drawInBatch(fontrenderer, s1, -fontrenderer.width(s1) / 2.0f, 1, 0xFFFFFFFF, false, mat, ctx.bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
        ctx.poseStack.translate(0.0F, 8F, -0.1F);
        com.github.alexmodguy.alexscaves.client.ACClientCompat.drawInBatch(fontrenderer, s2, -fontrenderer.width(s2) / 2.0f, 1, 0xFFFFFFFF, false, mat, ctx.bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
