...
public class AdvancedPathNavigate extends AbstractAdvancedPathNavigate {
    ...
    public void tick() {
        ...
        if (!this.isDone()) {
            ...
            // 1.21.9 deleted DebugPackets entirely: the F3 path overlay is now driven by the
            // DebugSubscriptions.ENTITY_PATHS subscription, which vanilla's own PathNavigation
            // publishes for every mob from its TrackingDebugSynchronizer — so from there on this
            // navigation's path reaches the debug renderer without being pushed, and there is
            // nothing to call. Spelled fully qualified because the import cannot be gated.
            //? if <1.21.9
            net.minecraft.network.protocol.game.DebugPackets.sendPathFindingPacket(this.level, this.mob, this.path, this.maxDistanceToWaypoint);
            if (!this.isDone()) {
    ...
    private boolean handleLadders(int oldIndex) {
        ...
        if (!this.isDone()) {
            ...
            if (pEx.isOnLadder() && pExNext != null && (pEx.y != pExNext.y || mob.getY() > pEx.y) && com.github.alexmodguy.alexscaves.server.misc.ACCompat.isLadder(level.getBlockState(pos), level, pos, ourEntity)) {
    ...
    private boolean handlePathPointOnLadder(final PathPointExtended pEx) {
        ...
        if (vec3.distanceToSqr(ourEntity.getX(), vec3.y, ourEntity.getZ()) < 0.6 && Math.abs(vec3.y - entityPos.getY()) <= 2.0) {
            ...
            if (newSpeed > 0) {
                ...
                if (com.github.alexmodguy.alexscaves.server.misc.ACCompat.isLadder(level.getBlockState(entityPos.below()), level, entityPos.below(), ourEntity)) {
