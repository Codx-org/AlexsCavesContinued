...
public class SurfaceRulesManager {
    ...
    public static Map<String, SurfaceRules.RuleSource> getOverworldRulesByBiomeForTerrablender(boolean vanilla) {
        ...
        for (SurfaceRules.RuleSource ruleSource : OVERWORLD_REGISTRY) {
            // 26.2 made BiomeConditionSource a record over a HolderSet<Biome> rather than a
            // List<ResourceKey<Biome>>, so the field is reached through the accessor and a key comes
            // back out of the Holder. Nothing AC itself registers matches on that node — from 26.2 it
            // supplies its own ACBiomeConditionSource, because vanilla's needs a HolderGetter this
            // code path has no way to obtain — but another mod registering through this manager still
            // can, and TerraBlender has no 26.2 build, so the scan stays rather than being stubbed.
            //? if >=26.2 {
