...
//             and the draw call. Both of vanilla Model's renderToBuffer overloads are final from
//             1.21.2, so there is no packed-int bridge to write here — the compat renderer calls
//             the eight-float form below directly. All that is left is the parts() traversal.
//? if >=1.21.2 {
/*public abstract class BasicEntityModel<T extends Entity> extends com.github.alexmodguy.alexscaves.client.render.compat.EntityModel<T> {
...
/*public abstract class BasicEntityModel<T extends Entity> extends EntityModel<T> {
    ...
    // Vanilla's abstract method on 1.21+. Splitting the packed colour back into channels here
    // keeps the whole model tree below on the four-float shape it was written for.
    @Override
    public void renderToBuffer(PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int color) {
        this.renderToBuffer(poseStack, consumer, light, overlay,
                com.github.alexmodguy.alexscaves.server.misc.ACColors.redF(color),
                com.github.alexmodguy.alexscaves.server.misc.ACColors.greenF(color),
                com.github.alexmodguy.alexscaves.server.misc.ACColors.blueF(color),
                com.github.alexmodguy.alexscaves.server.misc.ACColors.alphaF(color));
    }
