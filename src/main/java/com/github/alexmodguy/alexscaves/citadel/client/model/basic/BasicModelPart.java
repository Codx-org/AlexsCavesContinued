...
public class BasicModelPart {
    ...
    private void doRender(PoseStack.Pose matrixEntryIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
        ...
        // Deliberately NOT named matrix3f: this is one of only two places in the tree that does real
        // 3x3 math with the normal matrix (vector3f.mul below) rather than handing it to
        // VertexConsumer#normal, so it must stay a Matrix3f on every version. The >=1.20.5
        // `Matrix3f matrix3f` -> PoseStack.Pose replacement would otherwise rewrite it. The other
        // one is AdvancedModelBox#doRender, whose local is already uniquely named.
        Matrix3f normalMatrix = matrixEntryIn.normal();
        ...
        for (BasicModelPart.ModelBox BasicModelPart$modelbox : this.cubeList) {
            for (BasicModelPart.TexturedQuad BasicModelPart$texturedquad : BasicModelPart$modelbox.quads) {
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    com.github.alexmodguy.alexscaves.client.ACClientCompat.emitVertex(bufferIn, vector4f.x(), vector4f.y(), vector4f.z(), red, green, blue, alpha, BasicModelPart$positiontexturevertex.textureU, BasicModelPart$positiontexturevertex.textureV, packedOverlayIn, packedLightIn, f, f1, f2);
                }
            }
        }
