...
public class UiRenderMacros {
    ...
    public static void drawLineRectGradient(final PoseStack ps,
        ...
        BufferBuilder buffer = ACClientCompat.beginImmediate(ACClientCompat.ImmediateDraw.POSITION_COLOR_FAN);
        ...
        ACClientCompat.drawImmediate(ACClientCompat.ImmediateDraw.POSITION_COLOR_FAN, buffer);

        buffer = ACClientCompat.beginImmediate(ACClientCompat.ImmediateDraw.POSITION_COLOR_FAN);
    ...
    public static void line(final PoseStack ps,
        ...
        final Matrix4f m = ps.last().pose();
        BufferBuilder buffer = ACClientCompat.beginImmediate(ACClientCompat.ImmediateDraw.POSITION_COLOR_LINES);
        buffer.vertex(m, x, y, 0).color(red, green, blue, alpha).endVertex();
        buffer.vertex(m, xEnd, yEnd, 0).color(red, green, blue, alpha).endVertex();
        ACClientCompat.drawImmediate(ACClientCompat.ImmediateDraw.POSITION_COLOR_LINES, buffer);
    ...
    public static void blit(final PoseStack ps,
                            ...
                            final float vMax) {
        final Matrix4f m = ps.last().pose();
        BufferBuilder buffer = ACClientCompat.beginImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_FAN, rl);
        ...
        ACClientCompat.drawImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_FAN, buffer, rl);
    }
    ...
    protected static void blitRepeatable(final PoseStack ps,
        ...
        BufferBuilder buffer = ACClientCompat.beginImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_TRIANGLES, rl);
        ...
        ACClientCompat.drawImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_TRIANGLES, buffer, rl);
    }
    ...
    public static void drawEntity(final PoseStack poseStack, final int x, final int y, final double scale,
        ...
        ACClientCompat.overrideCameraOrientation(dispatcher, pitchRotation);
        ACClientCompat.setDispatcherRenderShadow(dispatcher, false);
        // Spelled through the imported Minecraft rather than the local `mc`, deliberately: from 26.2
        // there is no RenderBuffers immediate side left and this whole chain is rewritten by the
        // !mc262-buffersource rule, which carries the receiver with it so it cannot leave a dangling
        // `mc.` behind. It must be the SHORT form — the rule's source starts at `Minecraft`, so a
        // fully-qualified spelling would leave `net.minecraft.client.` stranded in front of a static
        // call. Every other site in the tree already spelled it this way; this was the one that did not.
        final MultiBufferSource.BufferSource buffers = Minecraft.getInstance().renderBuffers().bufferSource();
        ACClientCompat.runAsFancy(() -> com.github.alexmodguy.alexscaves.client.ACClientCompat.dispatcherRender(dispatcher, entity, 0.0D, 0.0D, 0.0D, 0.0F, 1.0F, poseStack, buffers, 0x00F000F0));
        buffers.endBatch();
        ACClientCompat.setDispatcherRenderShadow(dispatcher, true);
