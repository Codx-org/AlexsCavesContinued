...
public class LocalEntityTickRateModifier extends LocalTickRateModifier {
    ...
    public LocalEntityTickRateModifier(CompoundTag tag) {
        super(tag);
        this.entityId = ACCompat.getInt(tag, "EntityId");
        EntityType type = BuiltInRegistries.ENTITY_TYPE.get(ResourceLocation.parse(ACCompat.getString(tag, "EntityType")));
    ...
    public boolean isEntityValid(Level level) {
        Entity entity = level.getEntity(this.entityId);
        return entity != null && com.github.alexmodguy.alexscaves.server.misc.ACCompat.isAddedToWorld(entity) && entity.getType().equals(expectedEntityType) && entity.isAlive() && (!(entity instanceof IModifiesTime) || ((IModifiesTime) entity).isTimeModificationValid(this));
    }
