...
public class SpelunkeryTableScreen extends AbstractContainerScreen<SpelunkeryTableMenu> {
    ...
    // 1.21.9 made a style's font a FontDescription rather than a bare id, so that a style can name
    // a font that is not a resource. The plain resource case is the record below, wrapping the very
    // same id.
    //? if >=1.21.9 {
    /*protected static final Style GLYPH_FONT = Style.EMPTY.withFont(new net.minecraft.network.chat.FontDescription.Resource(ResourceLocation.fromNamespaceAndPath("minecraft", "alt")));
    *///?} else {
    ...
    // 26 made imageWidth/imageHeight final and hands them to the constructor instead. The five-argument
    // super has existed since long before that, but assigning the fields is the only form that works
    // below 26, so the two spellings are gated rather than unified.
    //? if >=26 {
    ...
    *///?} else {
    ...
    //
    // 26 renamed the whole draw chain to extract* and made the base class drive it: extractRenderState
    // reaches the background (through Screen), the labels, the slots and the tooltip on its own, so the
    // four calls this override used to make itself are all super's now and only the three the base class
    // knows nothing about remain. The three arms have to be flat — Stonecutter cannot nest the 1.20.2
    // gate inside the 26 one.
    //? if >=26 {
    /*@Override
    public void extractRenderState(GuiGraphics guiGraphics, int x, int y, float partialTick) {
    ...
    *///?} else {
    ...
    private void renderDescText(GuiGraphics guiGraphics) {
        ...
        if (invalidTablet) {
            ...
            for (int tally = 1; tally <= attemptsLeft; tally++) {
                if (tally % 5 == 0) {
                    ACClientCompat.blit(guiGraphics, WIDGETS_TEXTURE, i + 10 + tallySpace - 22, j + 70, 3, 52, 27, 14);
                    tallySpace += 7;
                } else {
                    ACClientCompat.blit(guiGraphics, WIDGETS_TEXTURE, i + 10 + tallySpace, j + 70, 0, 52, 3, 14);
                    ACClientCompat.blit(guiGraphics, WIDGETS_TEXTURE, i + 10 + tallySpace, j + 70, 0, 52, 3, 14);
                    tallySpace += 4;
                }
            }
    ...
    //? if >=26 {
    /*@Override
    protected void extractTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
    *///?} else {
    @Override
        ...
        if (isFirstTimeUsing() && tutorialStep < 6) {
            ...
            if (tutorialStep == 0) {
                ...
                if (mouseX > i + exclaimX - 5 && mouseY > j + exclaimY - 5 && mouseX < i + exclaimX + 15 && mouseY < j + exclaimY + 15) {
                    ...
                    ACClientCompat.renderTooltip(guiGraphics, font, step1Tooltip, Optional.empty(), mouseX, mouseY);
                }
            ...
            ACClientCompat.blit(guiGraphics, WIDGETS_TEXTURE, i + exclaimX, j + exclaimY, tickCount % 20 < 10 ? 7 : 0, 70, 7, 16);
    ...
    /**
     * The magnifying glass, at the position it has lerped to this frame.
     *
     * <p>1.21.6 addresses a GUI quad in whole pixels, so the fractional part of that lerp moves into
     * the transform — dropping it instead would make the glass jump a pixel at a time as it is
     * dragged. The z it used to be drawn at goes away with the immediate draw: the GUI is batched and
     * layered by submission order now, and this is the last thing the screen draws either way.
     */
    private void renderMagnify(GuiGraphics guiGraphics, float partialTick) {
        //? if >=1.21.6 {
        /*float lerpX = getMagnifyPosX(ACClientCompat.frameTime());
        float lerpY = getMagnifyPosY(ACClientCompat.frameTime());
        ACClientCompat.pushPose(guiGraphics);
        ACClientCompat.translate(guiGraphics, lerpX, lerpY);
        guiGraphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, WIDGETS_TEXTURE, 0, 0, 0.0F, 14.0F, 38, 38, 38, 38, 256, 256, -1);
        ACClientCompat.popPose(guiGraphics);
        *///?} else {
        BufferBuilder bufferbuilder = ACClientCompat.beginImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_QUADS, WIDGETS_TEXTURE);
        Matrix4f matrix4f = guiGraphics.pose().last().pose();
        float actualPartialTick = ACClientCompat.frameTime();
        ...
        ACClientCompat.drawImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_QUADS, bufferbuilder, WIDGETS_TEXTURE);
        //?}
    }

    // Same reordering as NuclearFurnaceScreen: renderBg became Screen#extractBackground on 26, with the
    // arguments in render order and the blur/darken pass reached through super.
    //? if >=26 {
    /*@Override
    ...
    *///?} else {
    ...
    //?}
        ...
        for (int bulb = 0; bulb < Math.min(level, 3); bulb++) {
            ACClientCompat.blit(guiGraphics, WIDGETS_TEXTURE, i + 92 + bulb * 15, j + 143, 0, 0, 13, 14);
        }
        if (hasPaper()) {
            ACClientCompat.blit(guiGraphics, WIDGETS_TEXTURE, i - 80, j + 10, 176, 0, 80, 149);
        }
        int tablet = hasTablet() ? attemptsLeft <= 1 ? 2 : 1 : 0;
        if (tablet > 0) {
            ACClientCompat.blit(guiGraphics, TABLET_TEXTURE, i + 20, j + 19, 0, (tablet - 1) * 121, 168, 120);
        }
    }
    ...
    // 1.21.9 folded the (x, y, button, modifiers) quartet every mouse callback carried into one
    // MouseButtonEvent record, and gave mouseClicked a second argument saying whether this is the
    // second click of a double-click. Nothing below reads the button or the modifiers, so the only
    // change is where the coordinates come from.
    //? if >=1.21.9 {
