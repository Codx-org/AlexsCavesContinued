...
// EntityRenderer and LivingEntityRenderer are spelled fully qualified below and must NOT be
// imported: the !mc2102-render-import-{entity,living} rules would rewrite those imports to the
// compat shims, and the renderer this class pulls out of the dispatcher is vanilla's own.
...
public class HologramProjectorBlockRenderer<T extends HologramProjectorBlockEntity> implements BlockEntityRenderer<T> {
    ...
    //? if >=1.21.5 {
    /*public void render(T hologram, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn, net.minecraft.world.phys.Vec3 acCameraPos) {
    *///?} else {
    ...
    private static String getPlayerModelName(PlayerInfo playerInfo, UUID uuid) {
        return playerInfo == null ? ACClientPlatform.defaultSkinModelName(uuid) : ACClientPlatform.skinModelName(playerInfo);
    }


    private static ResourceLocation getPlayerSkinTextureLocation(PlayerInfo playerInfo, UUID uuid) {
        return playerInfo == null ? ACClientPlatform.defaultSkinTexture(uuid) : ACClientPlatform.skinTexture(playerInfo);
    }
    ...
    private static void renderPlayerHologram(UUID lastPlayerUUID, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int i) {
        ...
        net.minecraft.client.renderer.entity.EntityRenderer renderer = ACClientCompat.playerRenderer(manager, modelName);
        ...
        // 1.21.2 moved the baby flag off the model and onto the render state; the hologram builds
        // no state, and a player is never a baby anyway.
        //? if <1.21.2 {
        model.young = false;
        //?}
        if (renderer instanceof net.minecraft.client.renderer.entity.LivingEntityRenderer livingEntityRenderer) {
            ...
            ACClientCompat.renderToBuffer(model, poseStack, ivertexbuilder, 240, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
            poseStack.popPose();
        }
