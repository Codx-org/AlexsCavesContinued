...
public class VoidBeingCloudParticle extends ACCustomParticle {
    ...
    public VoidBeingCloudParticle(ClientLevel world, double x, double y, double z, int size, int target, int totalTendrils) {
        ...
        String textureName = "alexscavesvoid_particle/void_cloud_" + id;
        dynamicTexture = ACClientCompat.newDynamicTexture(textureName, textureSize, textureSize, true);
        ResourceLocation resourcelocation = ACClientCompat.registerDynamicTexture(textureName, dynamicTexture);
    ...
    private void updateTexture() {
        ...
        for (int i = 0; i < textureSize; ++i) {
            for (int j = 0; j < textureSize; ++j) {
                ...
                if (alpha < 0) {
                    ACClientCompat.setPixelABGR(this.dynamicTexture.getPixels(), j, i, 0);
                } else {
                    ACClientCompat.setPixelABGR(this.dynamicTexture.getPixels(), j, i, ACColors.argb((int) Math.min(alpha * 255, 255), black, black, black));
                }
            }
        }
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        float f6 = 1;
        int j = 240;
        vertexConsumer1.vertex(avector3f[0].x(), avector3f[0].y(), avector3f[0].z()).color(this.rCol, this.gCol, this.bCol, this.alpha).uv(f8, f6).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
        vertexConsumer1.vertex(avector3f[1].x(), avector3f[1].y(), avector3f[1].z()).color(this.rCol, this.gCol, this.bCol, this.alpha).uv(f8, f5).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
        vertexConsumer1.vertex(avector3f[2].x(), avector3f[2].y(), avector3f[2].z()).color(this.rCol, this.gCol, this.bCol, this.alpha).uv(f7, f5).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
        vertexConsumer1.vertex(avector3f[3].x(), avector3f[3].y(), avector3f[3].z()).color(this.rCol, this.gCol, this.bCol, this.alpha).uv(f7, f6).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(j).normal(matrix3f, 0.0F, -1.0F, 0.0F).endVertex();
