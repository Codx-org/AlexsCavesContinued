...
public class LicowitchPossessionLayer<T extends LivingEntity & PossessedByLicowitch, M extends AdvancedEntityModel<T>> extends RenderLayer<T, M> {
    ...
    // From 1.21.2 a RenderLayerParent is keyed on the render state, not the entity, so the parent
    // this layer is handed is the one the compat RenderLayer takes.
    public LicowitchPossessionLayer(
            //? if >=1.21.2 {
            /*RenderLayerParent<com.github.alexmodguy.alexscaves.client.render.compat.ACRenderState, M> renderLayerParent
            *///?} else {
    ...
    public LicowitchPossessionLayer(
            ...
            *///?} else {
            RenderLayerParent<T, M> renderLayerParent,
            //?}
            Function<T, ResourceLocation> replacementTexture) {
        super(renderLayerParent);
    ...
    public LicowitchPossessionLayer(
            ...
            *///?} else {
            RenderLayerParent<T, M> renderLayerParent,
            //?}
            AdvancedEntityModel<T> replacementModel, Function<T, ResourceLocation> replacementTexture) {
        super(renderLayerParent);
