...
public class CaveMapRenderer {
    ...
    public CaveMapRenderer(BlockPos target, int[] mapBiomes, long seed, boolean transparent, int index) {
        ...
        String textureName = "cave_map/" + index + (transparent ? "" : "_frame");
        this.texture = ACClientCompat.newDynamicTexture(textureName, 128, 128, true);
        ResourceLocation resourcelocation = ACClientCompat.registerDynamicTexture(textureName, this.texture);
        ...
        if (registry != null && mapBiomes.length >= 128 * 128) {
            for (int i = 0; i < 128; ++i) {
                for (int j = 0; j < 128; ++j) {
                    ...
                    int r = ACColors.abgrRed(biomeColor);
                    int g = ACColors.abgrGreen(biomeColor);
                    int b = ACColors.abgrBlue(biomeColor);
                    double edge = Math.sqrt((i - 64) * (i - 64) + (j - 64) * (j - 64)) / 128;
                    int alpha = transparent ? Math.max(255 - (int) (255F * (edge * edge)), 10) : 255;
                    ACClientCompat.setPixelABGR(this.texture.getPixels(), j, i, ACColors.abgr(alpha, b, g, r));
                }
            }
        }
        this.texture.upload();
