    ...
    public void render(CrushedBlockEntity entity, float f1, float f2, PoseStack stack, MultiBufferSource source, int i) {
        ...
        for (MovingBlockData data : entity.getData()) {
            ...
            if (blockstate.getRenderShape() != RenderShape.INVISIBLE) {
                ...
                // Blocks drawn by a block-entity renderer (chests and the like) used to come out of
                // renderSingleBlock via the item renderer, already facing north, so they needed rotating
                // back. 1.21.4 deleted that path — renderSingleBlock draws the baked model and nothing
                // else — so there is no longer anything here to rotate.
                //? if <1.21.4 {
                ...
                //?}
                ACClientCompat.renderSingleBlock(blockstate, stack, source, i, OverlayTexture.NO_OVERLAY);
                stack.popPose();
            }
        }
        stack.popPose();
