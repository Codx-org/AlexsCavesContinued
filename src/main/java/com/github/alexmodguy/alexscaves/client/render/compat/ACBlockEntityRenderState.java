...
// so the thirteen tile renderers in this mod keep their pre-1.21.9 bodies verbatim.
//
// Holding the BlockEntity across the extract/submit boundary is exactly what ACRenderState does for
// entities: it is only read during the same frame, on the render thread, and the alternative is
// rewriting thirteen renderers to copy every field they touch into a state object.
//? if >=1.21.9 {
/*public class ACBlockEntityRenderState extends net.minecraft.client.renderer.blockentity.state.BlockEntityRenderState {
