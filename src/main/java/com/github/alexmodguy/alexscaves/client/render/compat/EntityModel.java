 ...
 *
 * <p>1.21.2 changed it to a render state, which breaks the declarations in this mod that name
 ...
 *
 * <p>The base of this mod's whole model hierarchy — the vendored Citadel {@code BasicEntityModel} —
 * extends this on 1.21.2+, so every concrete model in the mod is one of these and the declarations
 * stay valid.
 *
 ...
 */
public abstract class EntityModel<T extends Entity> extends net.minecraft.client.model.EntityModel<ACRenderState> {
	 ...
	 *
	 * <p>{@link ACRenderState#chromeOnly} means the renderer is only re-entering vanilla's body to
	 * reach its leash/name-tag tail, so the animation must not run a second time.
	 */
	@Override
	public final void setupAnim(ACRenderState state) {
		if (state.chromeOnly) {
			return;
		}
