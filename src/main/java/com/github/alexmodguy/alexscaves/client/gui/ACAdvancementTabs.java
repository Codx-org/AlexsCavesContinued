...
public class ACAdvancementTabs {
    ...
    // Everything below talks in advancement IDs rather than in Advancements. 1.20.2 split the
    // advancement tree into Advancement (the data) and AdvancementNode (its place in the tree),
    // and the tab/widget classes went with the node — so passing an Advancement around would need
    // the parameter type itself gated in four places. The ID is the same on every version, and
    // ACClientPlatform.advancementId is the one place that knows how to get it.
    public static boolean isAlexsCavesWidget(ResourceLocation advancementId) {
    ...
    public static void renderTabBackground(GuiGraphics guiGraphics, int topX, int topY, DisplayInfo displayInfo, double scrollX, double scrollY) {
        ...
        int fastColor = ACColors.lerp(hoverProgress, previousHoverType.backgroundColor, hoverType.backgroundColor);
    ...
    public enum Type {
        ...
        private static Type getDirectType(ResourceLocation advancementId) {
            for (Type type : values()) {
                if (type.root.equals(advancementId)) {
                    return type;
                }
            }
            return DEFAULT;
        ...
        // Both walks climb the WIDGET tree rather than the advancement tree. The widget tree
        // mirrors it (a widget's parent is its advancement's nearest visible ancestor), it is
        // reachable without an Advancement in hand, and stopping at a null widget parent is what
        // the second walk already did.
        public static Type forAdvancement(AdvancementWidget advancementWidget) {
            Type direct = getDirectType(ACClientPlatform.advancementId(advancementWidget));
            AdvancementWidget next = advancementWidget;
            while (direct == DEFAULT && next.parent != null) {
                next = next.parent;
                direct = getDirectType(ACClientPlatform.advancementId(next));
            }
            return direct;
        }
