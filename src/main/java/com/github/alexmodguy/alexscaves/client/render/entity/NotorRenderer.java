...
public class NotorRenderer extends MobRenderer<NotorEntity, NotorModel> {
    ...
    public static <E extends Entity> void renderEntityInHologram(E entityIn, double x, double y, double z, float yaw, float partialTicks, PoseStack matrixStack, MultiBufferSource bufferIn, int packedLight) {
        ...
        // Deliberately Object, and deliberately net.minecraft's LivingEntityRenderer below rather
        // than the compat shim: from 1.21.2 the dispatcher's lookup is parameterised on a render
        // state, so the renderer type is unnameable, and the subject of a hologram may just as well
        // be a vanilla mob with a vanilla renderer.
        Object render = null;
        try {
            ...
            if (entityIn instanceof LivingEntity living) {
                ...
                entityIn.setYRot(0);
                entityIn.yRotO = 0;
                if (render instanceof net.minecraft.client.renderer.entity.LivingEntityRenderer renderer && renderer.getModel() != null) {
                    net.minecraft.client.model.EntityModel model = (net.minecraft.client.model.EntityModel) renderer.getModel();
                    ResourceLocation texture = ACClientCompat.rendererTexture(entityIn, partialTicks);
                    ...
                    // 1.21.2 moved the crouch flag onto the render state; the hologram poses the
                    // model itself from a neutral state, so there is nothing left to suppress.
                    //? if <1.21.2 {
                    ...
                    matrixStack.scale(-living.getScale(), -living.getScale(), living.getScale());
                    VertexConsumer ivertexbuilder = bufferIn.getBuffer(ACRenderTypes.getHologram(ACClientCompat.rendererTexture(entityIn, partialTicks)));
                    ACClientCompat.setupAnim(FerrouslimeRenderer.FERROUSLIME_MODEL, ferrouslime, false, false, 0.0F,
                            0.0F, 0.0F, -0.1F, 0.0F, 0.0F, partialTicks);
