...
public class CaveMapRenderHelper {
    ...
    // Upstream bound the player's skin to texture unit 0 here and in renderPlayerArm, copied from
    // vanilla's own renderMapHand. It has never had an effect: the arm is drawn by
    // PlayerRenderer#renderRightHand through a MultiBufferSource, whose RenderType names the skin
    // itself, so nothing ever read the binding. 1.21.5 deleted the ResourceLocation overload of
    // setShaderTexture, which is what surfaced it; the calls are simply gone on every version.
    private static void renderMapHand(PoseStack poseStack, MultiBufferSource bufferSource, int i, HumanoidArm humanoidArm) {
        ...
        ACClientCompat.renderFirstPersonHand(playerrenderer, poseStack, bufferSource, i, Minecraft.getInstance().player, humanoidArm);
    ...
    private static void renderPlayerArm(PoseStack poseStack, MultiBufferSource bufferSource, int i, float animation1, float animation2, HumanoidArm humanoidArm) {
        ...
        ACClientCompat.renderFirstPersonHand(playerrenderer, poseStack, bufferSource, i, abstractclientplayer, flag ? HumanoidArm.RIGHT : HumanoidArm.LEFT);
