 ...
 * dreadbow and friends, each falling back to a flat {@code *_SPRITE} item outside the hand contexts.
 *
 * <p>How it is <i>wired</i> changed at 1.21.4, which deleted {@code BlockEntityWithoutLevelRenderer}
 * and the whole ISTER mechanism. The body below is untouched on every version; only the class header
 * and the constructor are gated. Below 1.21.4 the loaders' {@code IClientItemExtensions} hands this
 * class to the item pipeline directly; from 1.21.4 the item model definitions written by
 * {@code DataPackMigration.writeItemModelDefinitions} name a {@code minecraft:special} renderer that
 * routes back in here — see {@link ACItemSpecialRenderer}.
 */
public class ACItemstackRenderer
        //? if <1.21.4
        extends BlockEntityWithoutLevelRenderer
{
    ...
    public void renderByItem(ItemStack itemStackIn, ItemDisplayContext transformType, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (itemStackIn.is(ACItemRegistry.CAVE_MAP.get())) {
            ...
            ACCompat.setTag(spriteItem, ACCompat.getTag(itemStackIn));
        ...
        if (itemStackIn.is(ACItemRegistry.PRIMITIVE_CLUB.get())) {
            ...
            if (heldIn3d) {
                ...
                VertexConsumer vertexconsumer = ACClientCompat.armorFoilBuffer(bufferIn, RenderType.armorCutoutNoCull(PRIMITIVE_CLUB_TEXTURE), itemStackIn.hasFoil());
        ...
        if (itemStackIn.is(ACItemRegistry.LIMESTONE_SPEAR.get())) {
            ...
            if (heldIn3d) {
                ...
                VertexConsumer vertexconsumer = ACClientCompat.armorFoilBuffer(bufferIn, RenderType.armorCutoutNoCull(LIMESTONE_SPEAR_TEXTURE), itemStackIn.hasFoil());
        ...
        if (itemStackIn.is(ACItemRegistry.RAYGUN.get())) {
            ...
            boolean gamma = ACCompat.enchantLevel(itemStackIn, ACEnchantmentRegistry.GAMMA_RAY)  > 0;
        ...
        if (itemStackIn.is(ACItemRegistry.SEA_STAFF.get())) {
            ...
            if (heldIn3d) {
                ...
                VertexConsumer vertexconsumer = ACClientCompat.armorFoilBuffer(bufferIn, RenderType.armorCutoutNoCull(SEA_STAFF_TEXTURE), itemStackIn.hasFoil());
            ...
            if (heldIn3d) {
                ...
                if(Minecraft.getInstance().player != null && Minecraft.getInstance().player.isUsingItem() && pullAmount > 0 && type != null && Minecraft.getInstance().level != null){
                    if(renderedDreadbowArrow == null || renderedDreadbowArrow.getType() != type){
                        renderedDreadbowArrow = ACClientCompat.displayEntity(type, Minecraft.getInstance().level);
                    }
                    ...
                    ACClientCompat.dispatcherRender(manager, renderedDreadbowArrow, 0.0D, 0.0D, 0.0D, 0.0F, 1.0F, poseStack, bufferIn, DreadbowItem.isConvertibleArrow (renderedDreadbowArrow) ? (int) (Math.round(240 * (1F - pullAmount))) : combinedLightIn);
                    poseStack.popPose();
                }
        ...
        if (itemStackIn.is(ACItemRegistry.SUGAR_STAFF.get())) {
            ...
            if (heldIn3d) {
                ...
                VertexConsumer vertexconsumer = ACClientCompat.armorFoilBuffer(bufferIn, RenderType.armorCutoutNoCull(SUGAR_STAFF_TEXTURE), itemStackIn.hasFoil());
        ...
        if (itemStackIn.is(ACItemRegistry.FROSTMINT_SPEAR.get())) {
            ...
            if (heldIn3d) {
                ...
                VertexConsumer vertexconsumer1 = ACClientCompat.armorFoilBuffer(bufferIn, RenderType.entityCutoutNoCull(FROSTMINT_SPEAR_TEXTURE), itemStackIn.hasFoil());
    ...
    private void renderStaticItemSprite(ItemStack spriteItem, ItemDisplayContext transformType, int combinedLightIn, int combinedOverlayIn, PoseStack poseStack, MultiBufferSource bufferIn, ClientLevel level) {
        if(sepiaFlag){
            ItemWidget.renderSepiaItem(poseStack, spriteItem, Minecraft.getInstance().renderBuffers().bufferSource());
        }else{
            com.github.alexmodguy.alexscaves.client.ACClientCompat.renderItemStatic(spriteItem, transformType, transformType == ItemDisplayContext.GROUND ? combinedLightIn : 240, combinedOverlayIn, poseStack, bufferIn, level, 0);
        }
    }
    ...
    private void renderStaticItemSpriteWithLighting(ItemStack spriteItem, ItemDisplayContext transformType, int combinedLightIn, int combinedOverlayIn, PoseStack poseStack, MultiBufferSource bufferIn, ClientLevel level) {
        if(sepiaFlag){
            ...
            com.github.alexmodguy.alexscaves.client.ACClientCompat.renderItemStatic(spriteItem, transformType, transformType != ItemDisplayContext.GUI ? combinedLightIn : 240, combinedOverlayIn, poseStack, bufferIn, level, 0);
        }
    }

    // 26.2 took the foil helpers private, onto the frame's own ItemFeatureRenderer, so there is no
    // static entry point left to call. ACDrawCollector reproduces the choice vanilla makes; the
    // `false` there is `sheeted`, matching the `noEntity = false` this passed on every older node.
    private static VertexConsumer getVertexConsumerFoil(MultiBufferSource bufferIn, RenderType _default, ResourceLocation resourceLocation, boolean foil){
        //? if >=26.2 {
        /*if (sepiaFlag) {
            return bufferIn.getBuffer(ACRenderTypes.getBookWidget(resourceLocation, true));
        }
        if (!foil) {
        ...
        return com.github.alexmodguy.alexscaves.client.render.compat.ACDrawCollector.foilBuffer(
                bufferIn::getBuffer, _default, false);
        *///?} else {
        return sepiaFlag ? bufferIn.getBuffer(ACRenderTypes.getBookWidget(resourceLocation, true)) : net.minecraft.client.renderer.entity.ItemRenderer.getFoilBuffer(bufferIn, _default, false, foil);
        //?}
    }
