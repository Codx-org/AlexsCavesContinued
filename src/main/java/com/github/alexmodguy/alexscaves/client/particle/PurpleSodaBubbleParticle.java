...
public class PurpleSodaBubbleParticle extends ACQuadParticle {
    ...
    public void tick() {
        ...
        if(level.getFluidState(slightlyAbove).is(ACTagRegistry.PURPLE_SODA)){
            fluidHeight = level.getFluidState(slightlyAbove).getHeight(level, slightlyAbove);
            lastSodaBlock = slightlyAbove;
        }else if(level.getFluidState(slightlyBelow).is(ACTagRegistry.PURPLE_SODA)){
