...
// This layer is attached to EVERY living renderer in the game through EntityRenderersEvent.AddLayers,
// so on 1.21.2+ it must not be a compat RenderLayer — that one is bound to ACRenderState and would
// class-cast on the first vanilla mob. StateRenderLayer is the arbitrary-parent variant; it hands
// back the same ten-argument render, so the body below is unchanged.
//? if >=1.21.2 {
/*public class ACPotionEffectLayer extends com.github.alexmodguy.alexscaves.client.render.compat.StateRenderLayer {
*///?} else {
...
//?}
    ...
    public static void renderBubbledFluid(Minecraft p110726, PoseStack poseStack, ResourceLocation texture, boolean translate) {
        ...
        float f = net.minecraft.client.renderer.LightTexture.getBrightness(p110726.player.level().dimensionType(), p110726.player.level().getMaxLocalRawBrightness(blockpos));
        ACClientCompat.setImmediateTint(f, f, f, translate ? 0.3F : 1.0F);
        Matrix4f matrix4f = poseStack.last().pose();
        BufferBuilder bufferbuilder = ACClientCompat.beginImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_QUADS_BLEND, texture);
        if (translate) {
        ...
        ACClientCompat.drawImmediate(ACClientCompat.ImmediateDraw.POSITION_TEX_QUADS_BLEND, bufferbuilder, texture);
    }
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn, Entity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entity instanceof LivingEntity living) {
            if (living.hasEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get())) && AlexsCaves.CLIENT_CONFIG.radiationGlowEffect.get()) {
                PostEffectRegistry.renderEffectForNextTick(ClientProxy.IRRADIATED_SHADER);
                int level = living.getEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get())).getAmplifier() + 1;
                ...
                poseStack.pushPose();
                ACClientCompat.renderToBuffer(this.getParentModel(), poseStack, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), 1, 1F, 1, alpha);
                poseStack.popPose();
            }
            if (living.hasEffect(ACCompat.effect(ACEffectRegistry.BUBBLED.get())) && living.isAlive()) {
            ...
            if (living.hasEffect(ACCompat.effect(ACEffectRegistry.DARKNESS_INCARNATE.get())) && AlexsCaves.CLIENT_CONFIG.radiationGlowEffect.get() && living.isAlive()) {
                ...
                ACClientCompat.renderToBuffer(this.getParentModel(), poseStack, ivertexbuilder, 0, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), 0F, 0F, 0F, alpha);
                poseStack.popPose();
            }
            if (living.hasEffect(ACCompat.effect(ACEffectRegistry.SUGAR_RUSH.get())) && getParentModel() instanceof HumanoidModel<?>) {
                VertexConsumer ivertexbuilder = bufferIn.getBuffer(RenderType.entityCutoutNoCull(TEXTURE_SUGAR_RUSH));
                ACClientCompat.renderToBuffer(this.getParentModel(), poseStack, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), 1, 1F, 1, 1);
            }
        }
    }
