...
public class SugarRushSound extends AbstractTickableSoundInstance implements UnlimitedPitch{
    ...
    public boolean canPlaySound() {
        return !this.user.isSilent() && this.user.hasEffect(ACCompat.effect(ACEffectRegistry.SUGAR_RUSH.get()));
    }

    public void tick() {
        if (this.user.isAlive() && user.hasEffect(ACCompat.effect(ACEffectRegistry.SUGAR_RUSH.get()))) {
            this.x = (float) this.user.getX();
            this.y = (float) this.user.getY();
            this.z = (float) this.user.getZ();
