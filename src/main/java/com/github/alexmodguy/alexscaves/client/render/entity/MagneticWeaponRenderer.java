    ...
    public void render(MagneticWeaponEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource source, int i) {
        ...
        if (controller instanceof Player player) {
            ...
            if (controller == Minecraft.getInstance().player && Minecraft.getInstance().options.getCameraType().isFirstPerson()) {
                ...
                Vec3 vec3 = com.github.alexmodguy.alexscaves.client.ACClientCompat.nearPlane(this.entityRenderDispatcher.camera).getPointOnPlane((float) leftAmount * 0.525F, -0.7F);
                vec3 = vec3.scale(d7);
        ...
        com.github.alexmodguy.alexscaves.client.ACClientCompat.renderItemStatic(itemStack, ItemDisplayContext.THIRD_PERSON_RIGHT_HAND, i, OverlayTexture.NO_OVERLAY, poseStack, source, entity.level(), 0);
        poseStack.popPose();
