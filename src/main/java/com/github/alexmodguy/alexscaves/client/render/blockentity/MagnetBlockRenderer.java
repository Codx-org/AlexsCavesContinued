...
public class MagnetBlockRenderer<T extends MagnetBlockEntity> implements BlockEntityRenderer<T> {
    ...
    //? if >=1.21.5 {
    /*public void render(T magnet, float partialTicks, PoseStack stack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn, net.minecraft.world.phys.Vec3 acCameraPos) {
    *///?} else {
    ...
    //?}
        ...
        if (state.getValue(MagnetBlock.POWERED)) {
            ...
            ACClientCompat.renderSingleBlock(copy, stack, bufferIn, combinedLightIn, OverlayTexture.NO_OVERLAY);
            stack.popPose();
        }
