...
public class ConversionCrucibleBlockRenderer<T extends ConversionCrucibleBlockEntity> implements BlockEntityRenderer<T> {
    ...
    //? if >=1.21.5 {
    /*public void render(T crucible, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn, net.minecraft.world.phys.Vec3 acCameraPos) {
    *///?} else {
    ...
    //?}
        ...
        if(showItemProgress > 0.0F) {
            ...
            if(!displayStack.isEmpty()) {
                ...
                // Unlike the other quad-walk sites this one never recentred the model — it spins about
                // the block's own corner. The 1.21.4 render state always applies the -0.5, so the
                // compensating +0.5 keeps the item exactly where it has always been.
                //? if >=1.21.4 {
                /*poseStack.translate(0.5F, 0.5F, 0.5F);
                com.github.alexmodguy.alexscaves.client.render.item.ACItemRenderCompat.renderTinted(displayStack, crucible.getLevel(), net.minecraft.world.item.ItemDisplayContext.NONE, poseStack, bufferIn.getBuffer(rt), r, g, b, showItemProgress * 0.35F, 240, OverlayTexture.NO_OVERLAY);
                *///?} else {
            ...
            if(text != null){
                ...
                com.github.alexmodguy.alexscaves.client.ACClientCompat.drawInBatch8xOutline(Minecraft.getInstance().font, text.getVisualOrderText(), f, 0.0F, ACColors.argb(Mth.clamp((int) (showItemProgress * 255), 4, 255), 255, 255, 255), ACColors.argb((int) (Mth.clamp((int) (showItemProgress * 200), 4, 255)), (int) (r * 255), (int) (g * 255), (int) (b * 255)), poseStack.last().pose(), bufferIn, 240);
                poseStack.popPose();
            }
            poseStack.pushPose();
    ...
    public static void renderModel(PoseStack.Pose p_111068_, VertexConsumer p_111069_, float alpha, @Nullable BlockState p_111070_, BakedModel p_111071_, float p_111072_, float p_111073_, float p_111074_, int p_111075_, int p_111076_, ModelData modelData, net.minecraft.client.renderer.RenderType renderType) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            renderQuadList(p_111068_, p_111069_, p_111072_, p_111073_, p_111074_, alpha, com.github.alexmodguy.alexscaves.client.ACClientCompat.modelQuads(p_111071_, p_111070_, direction, randomsource, modelData, renderType), p_111075_, p_111076_);
        }

        randomsource.setSeed(42L);
        renderQuadList(p_111068_, p_111069_, p_111072_, p_111073_, p_111074_, alpha, com.github.alexmodguy.alexscaves.client.ACClientCompat.modelQuads(p_111071_, p_111070_, (Direction) null, randomsource, modelData, renderType), p_111075_, p_111076_);
    }
