...
public class WitchCookieParticle extends ACQuadParticle {
    ...
    protected WitchCookieParticle(ClientLevel world, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, SpriteSet spriteSet) {
        ...
        this.quadSize = 0.22F + world.getRandom().nextFloat() * 0.05F;
        this.gravity = -0.02F - 0.03F * level.getRandom().nextFloat();
        this.speedUpWhenYMotionIsBlocked = true;
        this.hasPhysics = true;
        this.xd = xSpeed + level.getRandom().nextFloat() * 0.1F - 0.05F;
        this.yd = ySpeed + 0.025F + level.getRandom().nextFloat() * 0.025F;
        this.zd = zSpeed + level.getRandom().nextFloat() * 0.1F - 0.05F;
        this.spriteSet = spriteSet;
        this.friction = 0.95F;
        this.lifetime = 18 + world.getRandom().nextInt(20);
        this.eatSpeed = Math.min(8 + world.getRandom().nextInt(12), lifetime - 5);
    }
