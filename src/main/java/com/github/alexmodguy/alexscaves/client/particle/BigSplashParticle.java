...
public class BigSplashParticle extends ACCustomParticle {
    ...
    public void tick() {
        ...
        if (k < 5) {
            ...
            for (int j = 0; j < scale * 2 + 1; j++) {
                Vec3 sputterFrom = new Vec3((level.getRandom().nextFloat() - 0.5F) * 0.1F * scale, -0.25F, (level.getRandom().nextFloat() - 0.5F) * 0.1F * scale).add(this.x, this.y, this.z);
                this.level.addParticle(ACParticleRegistry.BIG_SPLASH_EFFECT.get(), sputterFrom.x, sputterFrom.y, sputterFrom.z, (level.getRandom().nextFloat() - 0.5F) * 0.2F, 0.3F + level.getRandom().nextFloat() * 0.2F, (level.getRandom().nextFloat() - 0.5F) * 0.2F);
            }
