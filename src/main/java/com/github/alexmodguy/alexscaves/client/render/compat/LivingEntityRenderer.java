 ...
 *
 * <p>Two type parameters, not three: {@link ACRenderState} is fixed as the state type, so every
 * renderer declaration in this mod compiles unchanged.
 *
...
public abstract class LivingEntityRenderer<T extends LivingEntity, M extends EntityModel<?>>
		extends net.minecraft.client.renderer.entity.LivingEntityRenderer<T, ACRenderState, M>
		implements com.github.alexmodguy.alexscaves.client.render.entity.LivingEntityRendererAccessor {
	...
	// See the note on the sibling EntityRenderer for what 1.21.9 did to this entry point.
	//? if >=1.21.9 {
	/*@Override
	public void submit(ACRenderState state, PoseStack poseStack, net.minecraft.client.renderer.SubmitNodeCollector collector, net.minecraft.client.renderer.state.CameraRenderState camera) {
	...
	*///?} else {
	@Override
	public void render(ACRenderState state, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
	 ...
	 * out of the render state the dispatcher stashed in {@link #renderingState}. The renderers that
	 * only override hooks (texture, scale, rotations) get this body; the handful that fully
	...
	public void render(T entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
		ACRenderState state = this.renderingState;
	...
	private void renderChrome(ACRenderState state, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
		//? if >=1.21.9 {
	...
	/**
	 * The layer loop, for the renderers that reimplement {@link #render(LivingEntity, float, float,
	 * PoseStack, MultiBufferSource, int)} wholesale and so never reach the call in that body.
	 *
	 * <p>They must not walk {@link #layers} themselves. A layer on this renderer is only ever a
	 * <em>vanilla</em> {@code RenderLayer<ACRenderState, M>}: besides this mod's compat layers the
	 * list also holds whatever this mod attaches to every living renderer through
	 * {@code EntityRenderersEvent.AddLayers} (a {@link StateRenderLayer}) and whatever other mods
	 * add. Casting the elements down to the compat {@link RenderLayer} to reach its ten-argument
	 * {@code render} therefore {@code ClassCastException}s as soon as such a mob comes into view.
	 */
	...
	/**
	 * The layer loop. From 1.21.9 a layer submits rather than renders, so it wants the collector this
	 * shim's recorder is wrapping; the recorder is flushed first so the model's own geometry is
	 * submitted ahead of the layers', preserving the pre-1.21.9 draw order.
	 */
	private void renderLayers(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, ACRenderState state) {
		//? if >=1.21.9 {
		/*ACSubmitBuffers buffers = ACSubmitBuffers.of(bufferSource);
		if (buffers == null) {
			return;
		...
		buffers.flush();
		for (net.minecraft.client.renderer.entity.layers.RenderLayer<ACRenderState, M> layer : this.layers) {
	...
	@Override
	public final ResourceLocation getTextureLocation(ACRenderState state) {
		return this.getTextureLocation(this.entityOf(state));
	}

	@Override
	@Nullable
	protected final RenderType getRenderType(ACRenderState state, boolean bodyVisible, boolean translucent, boolean glowing) {
		return state.chromeOnly ? null : this.getRenderType(this.entityOf(state), bodyVisible, translucent, glowing);
	}

	@Override
	protected final boolean shouldRenderLayers(ACRenderState state) {
		return !state.chromeOnly;
	}

	@Override
	protected final boolean isBodyVisible(ACRenderState state) {
		return this.isBodyVisible(this.entityOf(state));
	}

	@Override
	protected final boolean isShaking(ACRenderState state) {
		return this.isShaking(this.entityOf(state));
	}

	@Override
	protected final float getWhiteOverlayProgress(ACRenderState state) {
	 ...
	 */
	@Override
	protected final void scale(ACRenderState state, PoseStack poseStack) {
	}

	/** Likewise — {@link #render} calls the legacy six-argument form directly. */
	@Override
	protected final void setupRotations(ACRenderState state, PoseStack poseStack, float bodyRot, float scale) {
	}

	// ---------------------------------------------------------------------------------------
	// The pre-1.21.2 hooks the renderers in this mod actually override.
	// ---------------------------------------------------------------------------------------
	...
	/**
	 * The notor's hologram scales its subject with the subject's own renderer. Pre-1.21.2 that goes
	 * through a mixin on vanilla {@code LivingEntityRenderer} (which can still shadow the
	 * entity-taking {@code scale}); here the hook is a plain override, because this class is the one
	 * that kept the entity-taking form.
	 */
	@Override
	public void scaleForHologram(LivingEntity entity, PoseStack poseStack, float partialTicks) {
		@SuppressWarnings("unchecked")
		T casted = (T) entity;
		this.scale(casted, poseStack, partialTicks);
	}
	 ...
	 * {@link #getFlipDegrees()} / {@link #isShaking(ACRenderState)}, which bridge back to the
	...
	protected void renderNameTag(T entity, net.minecraft.network.chat.Component name, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
		// See the note on the sibling EntityRenderer bridge: submitNameTag reads the text off the
	...
	// ---------------------------------------------------------------------------------------
	// Culling. Repeated from the sibling EntityRenderer shim rather than inherited, because this
	// class extends vanilla's LivingEntityRenderer directly — see the class javadoc.
	// ---------------------------------------------------------------------------------------
	...
	@SuppressWarnings("unchecked")
	private T entityOf(ACRenderState state) {
