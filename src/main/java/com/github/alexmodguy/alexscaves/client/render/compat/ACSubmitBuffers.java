...
// flush(), hands one callback per RenderType to the collector to replay it. This mod's renderers,
// layers, models and block-entity renderers therefore keep their pre-1.21.2 shape, exactly as they
// did across the 1.21.2 render-state rewrite.
//
...
//
// Ported from the sibling AlexsMobsContinued tree, which took the 1.21.9 wave first.
//? if >=1.21.9 {
...
// Deliberately NOT final: from 26.2 there is no vanilla MultiBufferSource left to implement, so this
// package vendors the interface, and the vendored MultiBufferSource.BufferSource — the immediate-mode
// handle a dozen legacy draw sites still ask for — is a subclass of this. That is what keeps
// ACSubmitBuffers#of(...) answering for those sites too, so a nested entity render or a name tag
// reached through one of them still finds a collector rather than silently drawing nothing.
public class ACSubmitBuffers implements MultiBufferSource {
    ...
    public ACSubmitBuffers(SubmitNodeCollector collector, CameraRenderState camera) {
        this.collector = collector;
        this.camera = camera;
    }

    // Some entry points hand out a collector but no camera state — the render layers, the block
    // entity renderers, and the loader render events. The frame's own CameraRenderState lives on
    // LevelRenderer#levelRenderState, which is private, so those sites pass null and camera()
    // rebuilds an equivalent one on demand from the live Camera.
    public ACSubmitBuffers(SubmitNodeCollector collector) {
        this(collector, null);
    }
    ...
    public static ACSubmitBuffers of(MultiBufferSource source) {
        return source instanceof ACSubmitBuffers buffers ? buffers : null;
    }
    ...
    public static SubmitNodeCollector collectorOf(MultiBufferSource source) {
        ACSubmitBuffers buffers = of(source);
        return buffers == null ? null : buffers.collector();
    }

    // Overridable so the vendored BufferSource can resolve the frame's collector lazily instead of
    // capturing one at construction — it is a long-lived singleton, the collector is per-frame.
    ...
    public void flush() {
        ...
        // A null collector means something drew outside the frame's submission phase — only reachable
        // through the vendored BufferSource, which hands one out on demand rather than being handed
        // one. Discard rather than throw: the alternative is an NPE deep inside an unrelated renderer.
        SubmitNodeCollector target = this.collector();
        if (target != null) {
            for (Map.Entry<RenderType, Recorder> entry : this.recorders.entrySet()) {
                Recorder recorder = entry.getValue();
                if (recorder.count > 0) {
                    target.submitCustomGeometry(IDENTITY, entry.getKey(), recorder);
                }
            }
        }
    ...
    private static final class Recorder implements VertexConsumer, SubmitNodeCollector.CustomGeometryRenderer {
        ...
        public VertexConsumer setColor(int red, int green, int blue, int alpha) {
            if (this.count > 0) {
                // Packed by hand rather than through ARGB: the `!mc21-vc-color` replacement rewrites
