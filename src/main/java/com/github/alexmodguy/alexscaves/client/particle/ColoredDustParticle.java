...
public class ColoredDustParticle extends ACQuadParticle {
    ...
    protected ColoredDustParticle(ClientLevel world, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
        ...
        this.quadSize = 0.5F + world.getRandom().nextFloat() * 0.2F;
        this.lifetime = 10 + world.getRandom().nextInt(4);
        this.friction = 0.96F;
    ...
    public static class SmallFactory implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            ColoredDustParticle particle = new ColoredDustParticle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed);
            particle.pickSprite(spriteSet);
            particle.quadSize = 0.2F + worldIn.getRandom().nextFloat() * 0.15F;
            particle.lifetime = 15 + worldIn.getRandom().nextInt(15);
            particle.xd = 0.0F;
            particle.yd = 0.1F * worldIn.getRandom().nextFloat();
            particle.zd = 0.0F;
