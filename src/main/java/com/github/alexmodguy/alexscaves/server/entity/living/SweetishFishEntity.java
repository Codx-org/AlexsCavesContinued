...
public class SweetishFishEntity extends WaterAnimal implements Bucketable, HasGummyColors {
    ...
    protected PathNavigation createNavigation(Level level) {
        return new WaterBoundPathNavigation(this, level) {
            ...
            // See RadgillEntity: PathNavigation#isInLiquid is gone from 1.20.2 on, so this overrides
            // its only caller instead.
            protected boolean canUpdatePath() {
                return SweetishFishEntity.this.isInBreathableFluid();
            }
        };
    }
    ...
    public static boolean checkSweetishFishSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, MobSpawnType spawnType, BlockPos pos, RandomSource randomSource) {
        return spawnType == MobSpawnType.SPAWNER || !level.getFluidState(pos).isEmpty() && level.getFluidState(pos).is(ACTagRegistry.PURPLE_SODA);
    }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setGummyColor(GummyColors.fromOrdinal(ACCompat.getInt(compound, "GummyColor")));
    }
    ...
    private boolean isInBreathableFluid() {
        return ACCompat.isInWaterOrBubble(this) || this.isInSoda();
    }
    ...
    public void tick(){
        ...
        boolean grounded = !isInBreathableFluid();
    ...
    public void travel(Vec3 travelVector) {
        if (this.isEffectiveAi() && this.isInBreathableFluid()) {
    ...
    *///?} else {
    ...
    private class WanderGoal extends Goal {
        ...
        private boolean isLiquidAt(BlockPos pos) {
            ...
            return state.is(FluidTags.WATER) || state.is(ACTagRegistry.PURPLE_SODA);
        }
        ...
        public boolean canUse() {
            if (!SweetishFishEntity.this.isInBreathableFluid()) {
                return false;
        ...
        public boolean canContinueToUse() {
            return SweetishFishEntity.this.isInBreathableFluid() && !SweetishFishEntity.this.navigation.isDone() && timeout < 40;
        }
