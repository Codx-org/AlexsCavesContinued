...
public class VallumraptorEntity extends DinosaurEntity implements IAnimatedEntity, ICustomCollisions, PackAnimal, ChestThief, TargetsDroppedItems {
    ...
    private static final Predicate<LivingEntity> VALLUMRAPTOR_TARGETS = living -> living.getType().builtInRegistryHolder().is(ACTagRegistry.VALLUMRAPTOR_TARGETS);
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return ACCompat.createEntity(ACEntityRegistry.VALLUMRAPTOR.get(), level);
    }
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return ACCompat.temptable(Monster.createMonsterAttributes().add(Attributes.ATTACK_DAMAGE, 3.0D).add(Attributes.MOVEMENT_SPEED, 0.2D).add(Attributes.FOLLOW_RANGE, 32.0D).add(Attributes.MAX_HEALTH, 28.0D));
    }
        ...
        if (b == 45) {
            ...
            if (!itemstack.isEmpty()) {
                for (int i = 0; i < 8; ++i) {
                    ...
                    this.level().addParticle(ACCompat.itemParticle(ParticleTypes.ITEM, itemstack), this.getX() + headPos.x, this.getY(0.5) + headPos.y, this.getZ() + headPos.z, (random.nextFloat() - 0.5F) * 0.1F, random.nextFloat() * 0.15F, (random.nextFloat() - 0.5F) * 0.1F);
                }
            }
        } else {
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setElder(ACCompat.getBoolean(compound, "Elder"));
        this.setRelaxedForTime(ACCompat.getInt(compound, "RelaxedTime"));
        this.justLootedChest = ACCompat.getBoolean(compound, "JustLootedChest");
    }
    ...
    *///?} else {
    ...
    public boolean canTargetItem(ItemStack stack) {
        return (stack.is(ACTagRegistry.VALLUMRAPTOR_STEALS) || ACCompat.isEdible(stack) && ACCompat.isMeat(stack)) && !stack.is(ACBlockRegistry.VALLUMRAPTOR_EGG.get().asItem());
    }
    ...
    public void onGetItem(ItemEntity e) {
        ...
        if (this.getAnimation() == ANIMATION_GRAB && this.getAnimationTick() > 15) {
            if (!this.getItemInHand(InteractionHand.MAIN_HAND).isEmpty() && !this.level().isClientSide()) {
                ACCompat.spawnAtLocation(this, this.getItemInHand(InteractionHand.MAIN_HAND), 0.0F);
            }
            this.take(e, 1);
