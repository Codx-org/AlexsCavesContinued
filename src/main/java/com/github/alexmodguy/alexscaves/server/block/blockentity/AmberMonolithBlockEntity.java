...
public class AmberMonolithBlockEntity extends BlockEntity implements ACUpdatePacketReceiver {
    ...
    public static void tick(Level level, BlockPos blockPos, BlockState state, AmberMonolithBlockEntity entity) {
        ...
        if (!level.isClientSide()) {
            if (entity.spawnType == null && entity.findSpawnsCooldown-- <= 0 || !entity.hasDonePostBossSpawn && entity.isMigration()) {
                entity.findSpawnsCooldown = 40 + level.getRandom().nextInt(50);
    ...
    private boolean spawnMobs() {
        ...
        for (int l1 = 0; l1 < spawnCount; l1++) {
            ...
            for (int i2 = 0; !flag && i2 < 6; i2++) {
                ...
                if (spawnType.canSummon() && ACCompat.spawnPositionOk(spawnType, level, blockpos)) {
                    ...
                    Entity entity;
                    try {
                        entity = ACCompat.createEntity(spawnType, level);
                    ...
                    ACCompat.moveTo(entity, d0, (double) blockpos.getY(), d1, level.getRandom().nextFloat() * 360.0F, 0.0F);
                    if (entity instanceof Mob) {
                        ...
                        if (net.minecraftforge.event.ForgeEventFactory.checkSpawnPosition(mob, (ServerLevelAccessor) level, MobSpawnType.CHUNK_GENERATION)) {
                            spawngroupdata = ACCompat.finalizeSpawn(mob, (ServerLevelAccessor) level, ((ServerLevel) level).getCurrentDifficultyAt(mob.blockPosition()), MobSpawnType.CHUNK_GENERATION, spawngroupdata);
                            ...
                            for (int i = 0; i < maxDist; i++) {
                                Vec3 vec3 = center.add(distance.normalize().scale(distance.length() * (i / (float) maxDist))).add(level.getRandom().nextFloat() - 0.5F, level.getRandom().nextFloat() - 0.5F, level.getRandom().nextFloat() - 0.5F);
    ...
    private void generateSpawnData() {
        ...
        MobSpawnSettings.SpawnerData spawnerData = getDepopulatedEntitySpawnData(level, this.getBlockPos(), 4 + level.getRandom().nextInt(8), 64, forcedEntityList);
        if (spawnerData != null) {
            spawnType = spawnerData.type;
            int j = Math.max(spawnerData.maxCount - spawnerData.minCount, 0);
            spawnCount = j <= 0 ? spawnerData.minCount : level.getRandom().nextInt(j) + spawnerData.minCount;
        }
    ...
    private static boolean isEntitySpawnBlocked(MobSpawnSettings.SpawnerData settings, Level level, BlockPos pos, int range) {
        if(settings.type.builtInRegistryHolder().is(ACTagRegistry.AMBER_MONOLITH_SKIPS)){
            return true;
        }
    ...
    private static MobSpawnSettings.SpawnerData getEntitySpawnSettingsForBiome(Level level, BlockPos pos, List<EntityType<?>> forcedEntityTypes) {
        ...
        if (biome != null) {
            ...
            if(!forcedEntityTypes.isEmpty()){
                ...
                // unwrap() hands back the wrappers and the filtered list has to keep them intact.
                //? if >=1.21.5 {
                /*List<net.minecraft.util.random.Weighted<MobSpawnSettings.SpawnerData>> matching = new ArrayList<>();
                for(net.minecraft.util.random.Weighted<MobSpawnSettings.SpawnerData> unwrapped : spawnList.unwrap()){
                    if(forcedEntityTypes.contains(unwrapped.value().type())){
                        matching.add(unwrapped);
                    }
                }
                if(!matching.isEmpty()){
                    spawnList = WeightedRandomList.of(matching);
                }
                *///?} else {
            ...
            if (!spawnList.isEmpty()) {
                return spawnList.getRandom(level.getRandom()).get();
            }
        }
        return null;
    }
    ...
    public void onDataPacket(Connection net, ClientboundBlockEntityDataPacket packet) {
        if (packet != null && packet.getTag() != null) {
            ...
            this.spawnCount = ACCompat.getInt(packet.getTag(), "SpawnCount");
            this.spawnsMobIn = ACCompat.getInt(packet.getTag(), "SpawnMobsIn");
            this.hasDonePostBossSpawn = ACCompat.getBoolean(packet.getTag(), "PostBossSpawn");
        }
    }
    ...
    public void load(CompoundTag tag) {
        ...
        this.spawnCount = ACCompat.getInt(tag, "SpawnCount");
        this.spawnsMobIn = ACCompat.getInt(tag, "SpawnMobsIn");
        this.hasDonePostBossSpawn = ACCompat.getBoolean(tag, "PostBossSpawn");
    }
