...
public class CaveBiomeMapWorldWorker implements ACWorldWorkerManager.IWorker {
    ...
    public void onWorkComplete(@Nullable BlockPos biomeCorner) {
        if(!complete){
            CompoundTag tag = ACCompat.getOrCreateTag(map);
            if (biomeCorner != null) {
                ...
                ACCompat.sendSystemMessage(player, Component.translatable("item.alexscaves.cave_map.error", distance).withStyle(ChatFormatting.RED));
