...
public class TremorsaurusEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen, ITallWalker, CullingBoundsEntity {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return ACCompat.temptable(Monster.createMonsterAttributes().add(Attributes.ATTACK_DAMAGE, 14.0D).add(Attributes.MOVEMENT_SPEED, 0.2D).add(Attributes.KNOCKBACK_RESISTANCE, 0.9D).add(Attributes.FOLLOW_RANGE, 32.0D).add(Attributes.MAX_HEALTH, 150.0D).add(Attributes.ARMOR, 8.0D));
    }
    ...
    public void tick() {
        ...
        if (this.onGround() && !ACFluids.isInAnyFluid(this) && this.walkAnimation.speed() > 0.1F && !this.isBaby()) {
            ...
            if (riderHitEntity != null && this.getAnimation() == ANIMATION_BITE && this.getAnimationTick() > 10 && this.getAnimationTick() <= 12) {
                if (this.hasLineOfSight(riderHitEntity) && this.distanceTo(riderHitEntity) < this.getBbWidth() + riderHitEntity.getBbWidth() + 2.0D) {
                    ...
                    if (riderHitEntity instanceof LivingEntity living) {
                        ACCompat.knockback(living, 0.5D, this.getX() - riderHitEntity.getX(), this.getZ() - riderHitEntity.getZ());
                    }
                    riderHitEntity = null;
                }
            }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return ACCompat.createEntity(ACEntityRegistry.TREMORSAURUS.get(), level);
    }

    @Override
    public boolean wantsToAttack(LivingEntity living, LivingEntity owner) {
        if(living instanceof TremorsaurusEntity tremorsaurus && (tremorsaurus.getTameAttempts() > 0 || tremorsaurus.hasEffect(ACCompat.effect(ACEffectRegistry.STUNNED.get())))){
            return false;
        }
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger) && passenger instanceof LivingEntity living && !this.touchingUnloadedChunk()) {
            ...
            moveFunction.accept(passenger, this.getX() + seatOffset.x, this.getY() + seatOffset.y + ACPlatform.passengersRidingOffset(this, passenger) - maxLegSolverHeight, this.getZ() + seatOffset.z);
        } else {
    ...
    public boolean onFeedMixture(ItemStack itemStack, Player player) {
        if (itemStack.is(ACItemRegistry.SERENE_SALAD.get()) && this.hasEffect(ACCompat.effect(ACEffectRegistry.STUNNED.get()))) {
            this.removeEffect(ACCompat.effect(ACEffectRegistry.STUNNED.get()));
            ...
            if (type == 3) {
                if (this.getAnimation() == NO_ANIMATION || this.getAnimation() == null) {
                    ...
                    if (hitresult instanceof EntityHitResult entityHitResult) {
                        ...
                        if (this.getRandom().nextBoolean() && riderHitEntity.getBbWidth() < 2.0F || ACFlyingAnimals.isFlyingAnimal(riderHitEntity)) {
                            flag = true;
                        }
                    } else {
                        riderHitEntity = null;
