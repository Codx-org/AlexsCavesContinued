...
public class TeslaBulbBlockEntity extends BlockEntity {
        ...
        if (entity.exploding) {
            if (entity.explodeProgress == 10.0F) {
                ...
                if (entity.dummyBolt == null) {
                    ...
                    com.github.alexmodguy.alexscaves.server.misc.ACCompat.setLightningDamage(entity.dummyBolt, 1);
                    entity.dummyBolt.setVisualOnly(true);
                }
                for (LivingEntity entity1 : level.getEntitiesOfClass(LivingEntity.class, ACPlatform.encapsulating(blockPos.offset(-5, -5, -5), blockPos.offset(5, 5, 5)))) {
    ...
    public void load(CompoundTag tag) {
        super.load(tag);
        this.exploding = ACCompat.getBoolean(tag, "Exploding");
    }
