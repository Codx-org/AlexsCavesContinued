...
public class MagnetronEntity extends Monster implements CullingBoundsEntity, ACMultipartOwner {

    private static final EntityDataAccessor<CompoundTag> BLOCKSTATES = SynchedEntityData.defineId(MagnetronEntity.class, com.github.alexmodguy.alexscaves.server.misc.ACDataSerializers.COMPOUND_TAG);
    private static final EntityDataAccessor<CompoundTag> BLOCK_POSES = SynchedEntityData.defineId(MagnetronEntity.class, com.github.alexmodguy.alexscaves.server.misc.ACDataSerializers.COMPOUND_TAG);
        ...
        if (!this.isAlive() && shouldDropBlocks()) {
            if (this.isFormed()) {
                for (MagnetronPartEntity part : allParts) {
                    if (part.getBlockState() != null && ACCompat.gameRule(level(), GameRules.RULE_MOBGRIEFING, true)) {
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        this.setFormed(ACCompat.getBoolean(compound, "Formed"));
        if (ACCompat.contains(compound, "Blockstates", 10)) {
            this.entityData.set(BLOCKSTATES, ACCompat.getCompound(compound, "Blockstates"));
        }
        if (ACCompat.contains(compound, "BlockPoses", 10)) {
            this.entityData.set(BLOCK_POSES, ACCompat.getCompound(compound, "BlockPoses"));
        }
    }
    ...
    private List<BlockPos> getAllBlockPos() {
        ...
        if (data.contains("BlockPos")) {
            ListTag listTag = ACCompat.getList(data, "BlockPos", 10);
            for (int i = 0; i < listTag.size(); ++i) {
                CompoundTag innerTag = ACCompat.getCompound(listTag, i);
                list.add(ACCompat.posFromTag(innerTag));
            }
        }
        return list;
    }
    ...
    private void setAllBlockPos(List<BlockPos> list) {
        ...
        for (BlockPos pos : list) {
            listTag.add(ACCompat.posToTag(pos));
        }
        ...
        if (!level().isClientSide()) {
            ...
            for (MagnetronPartEntity entity : this.allParts) {
                if (entity.getBlockState() == null && entity.getStartPosition() != null) {
                    ...
                    saved.add(state);
                    if (ACCompat.gameRule(level(), GameRules.RULE_MOBGRIEFING, true) && !level().isClientSide()) {
    ...
    protected float getStandingEyeHeight(Pose pose, EntityDimensions dimensions) {
        return 0.55F * ACCompat.height(dimensions);
    }
    ...
    public boolean canBeAffected(MobEffectInstance effectInstance) {
        return super.canBeAffected(effectInstance) && ACCompat.vanillaEffect(effectInstance.getEffect()) != ACEffectRegistry.MAGNETIZING.get();
    }
