...
public class MovingBlockData {
    ...
    public MovingBlockData(Level level, CompoundTag tag) {
        this(NbtUtils.readBlockState(level.holderLookup(Registries.BLOCK), ACCompat.getCompound(tag, "BlockState")), getShapeFromTag(ACCompat.getCompound(tag, "VoxelShape")), new BlockPos(ACCompat.getInt(tag, "OffsetX"), ACCompat.getInt(tag, "OffsetY"), ACCompat.getInt(tag, "OffsetZ")), tag.contains("BlockData") ? ACCompat.getCompound(tag, "BlockData") : null);
    }
    ...
    private static VoxelShape getShapeFromTag(CompoundTag data) {
        ...
        if (data.contains("AABBs")) {
            ListTag listtag = ACCompat.getList(data, "AABBs", 10);
            for (int i = 0; i < listtag.size(); ++i) {
                CompoundTag innerTag = ACCompat.getCompound(listtag, i);
                AABB aabb = new AABB(ACCompat.getDouble(innerTag, "BoxMinX"), ACCompat.getDouble(innerTag, "BoxMinY"), ACCompat.getDouble(innerTag, "BoxMinZ"), ACCompat.getDouble(innerTag, "BoxMaxX"), ACCompat.getDouble(innerTag, "BoxMaxY"), ACCompat.getDouble(innerTag, "BoxMaxZ"));
