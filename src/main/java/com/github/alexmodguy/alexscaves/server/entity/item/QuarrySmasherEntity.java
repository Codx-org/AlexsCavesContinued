...
public class QuarrySmasherEntity extends Entity implements ACMultipartOwner {
        ...
        if (this.pullingItemsFor() > 0) {
            ...
            if (flag && quarry != null) {
                if (level().getBlockEntity(quarry) instanceof QuarryBlockEntity quarryBlockEntity) {
                    ...
                    level().playSound((Player) null, quarry, ACSoundRegistry.QUARRY_CRUSH.get(), SoundSource.BLOCKS, 1.0F, 0.9F + level().getRandom().nextFloat() * 0.2F);
                }
            }
            if (i == 0) {
                pulledItems.clear();
    ...
    *///?} else {
    ...
    public boolean hurt(DamageSource damageSource, float damageValue) {
        // Entity#isInvulnerableTo became protected final isInvulnerableToBase in 1.21.2; this class
        // has no override of its own, so the call simply moves to the new name.
        //? if >=1.21.2 {
            ...
            if (damageSustained >= 10) {
                ...
                if (!this.isRemoved()) {
                    for (int i = 0; i < 1 + random.nextInt(1); i++) {
                        ACCompat.spawnAtLocation(this, ACItemRegistry.AZURE_NEODYMIUM_INGOT.get());
                    }
                    for (int i = 0; i < 1 + random.nextInt(1); i++) {
                        ACCompat.spawnAtLocation(this, ACItemRegistry.SCARLET_NEODYMIUM_INGOT.get());
                    }
                }
