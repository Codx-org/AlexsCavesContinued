...
import com.github.alexmodguy.alexscaves.citadel.server.entity.pathfinding.raycoms.ITallWalker;
...
public class TremorzillaEntity extends DinosaurEntity implements KeybindUsingMount, IAnimatedEntity, ShakesScreen, KaijuMob, ActivatesSirens, ITallWalker, CullingBoundsEntity, ACMultipartOwner {
    ...
    private static final EntityDimensions SWIMMING_SIZE = ACCompat.dimensions(4.0F, 5.0F, true);
    ...
    public TremorzillaEntity(EntityType entityType, Level level) {
        ...
        dummyExplosion = ACPlatform.explosion(level(), null, this.getX(), this.getY(), this.getZ(), 10.0F, Explosion.BlockInteraction.DESTROY);
    }
    ...
    // 1.20.2 made LivingEntity#canBreatheUnderwater final and moved the answer into the
    // #minecraft:can_breathe_under_water entity-type tag, which this mod's data pack sets.
    //? if <1.20.2 {
    ...
    public void tick() {
        ...
        boolean water = ACFluids.isInAnyFluid(this);
        ...
        if (this.onGround() && !ACFluids.isInAnyFluid(this) && this.walkAnimation.speed() > 0.1F && !this.isBaby() && !this.isNoAi() && this.isAlive()) {
        ...
        if (this.hasEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()))) {
            MobEffectInstance instance = this.getEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()));
            int level = instance == null ? 1 : 1 + instance.getAmplifier();
            this.heal(level * 12);
            this.removeEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()));
        }
    ...
    private double getMaxFluidHeight() {
        return ACFluids.maxFluidHeight(this);
    }
    ...
    public void travel(Vec3 vec3d) {
        if (this.getAnimation() == ANIMATION_LEFT_STOMP || this.getAnimation() == ANIMATION_RIGHT_STOMP || this.getAnimation() == ANIMATION_LEFT_TAIL || this.getAnimation() == ANIMATION_RIGHT_TAIL || this.isFiring() && !this.isVehicle()) {
            vec3d = Vec3.ZERO;
            super.travel(vec3d);
        } else if (ACFluids.isInAnyFluid(this) && (this.isEffectiveAi() || this.isVehicle())) {
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            if (com.github.alexmodguy.alexscaves.server.misc.ACPlatform.canEntityGrief(this.level(), this) && blockBreakCounter <= 0) {
    ...
    public boolean isStunned() {
        return this.hasEffect(ACCompat.effect(ACEffectRegistry.STUNNED.get()));
    }

    public void readAdditionalSaveData(CompoundTag compound) {
        super.readAdditionalSaveData(compound);
        this.setCharge(ACCompat.getInt(compound, "Charge"));
        this.setSpikesDownAmount(ACCompat.getFloat(compound, "SpikesDownAmount"));
        this.wantsToUseBeamFromServer = ACCompat.getBoolean(compound, "ServerBeamTrigger");
        this.setTameAttempts(ACCompat.getInt(compound, "TameAttempts"));
    }
        ...
        if (level().isClientSide()) {
            for (int i = 0; i < 4; i++) {
                for (int i1 = 0; i1 < 10 + random.nextInt(10); i1++) {
                    ...
                    if (state.isSolid()) {
                        ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, state), center.x + extraX, ground.getY() + extraY, center.z + extraZ, motionX, motionY, motionZ);
                    }
                }
            }
        }
    ...
    public boolean hurtEntitiesAround(Vec3 center, float radius, float damageAmount, float knockbackAmount, boolean radioactive, boolean hurtsOtherKaiju, boolean stretchY) {
        ...
        for (LivingEntity living : level().getEntitiesOfClass(LivingEntity.class, aabb, EntitySelector.NO_CREATIVE_OR_SPECTATOR)) {
            if (!living.is(this) && !this.isAlliedTo(living) && living.getType() != this.getType() && living.distanceToSqr(center.x, stretchY ? living.getY() : center.y, center.z) <= radius * radius && (!radioactive || canEntityBeHurtByBeam(living, center)) && (hurtsOtherKaiju || !(living instanceof KaijuMob))) {
                if (ACCompat.hurt(living, damageSource, damageAmount)) {
                    flag = true;
                    ...
                    if (radioactive) {
                        ...
                        living.addEffect(new MobEffectInstance(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()), 6000, 2));
                    }
                }
            }
        }
    ...
    public void knockbackTarget(Entity target, double strength, double x, double z, boolean ignoreResistance) {
        // The loader hook is three spellings deep and Fabric has none at all, so it lives behind
        // ACPlatform rather than in four arms here; see ACPlatform#onLivingKnockBack.
        double[] knockback = com.github.alexmodguy.alexscaves.server.misc.ACPlatform.onLivingKnockBack(this, (float) strength, x, z);
        if (knockback == null) return;
        strength = knockback[0];
        x = knockback[1];
        z = knockback[2];
        if (!ignoreResistance) {
    ...
    public boolean breakBlocksAround(Vec3 center, float radius, boolean square, boolean triggerExplosions, float dropChance) {
        if (this.isBaby() || !com.github.alexmodguy.alexscaves.server.misc.ACPlatform.canEntityGrief(this.level(), this) || level().isClientSide()) {
            return false;
        }
        boolean flag = false;
        for (BlockPos blockpos : BlockPos.betweenClosed(Mth.floor(center.x - radius), Mth.floor(center.y - radius), Mth.floor(center.z - radius), Mth.floor(center.x + radius), Mth.floor(center.y + radius), Mth.floor(center.z + radius))) {
            ...
            if (!blockstate.is(ACTagRegistry.NUKE_PROOF) && blockstate.blocksMotion() && (blockstate.getBlock().getExplosionResistance() <= 15 || nuke) && (square || blockpos.distToCenterSqr(center.x, center.y, center.z) < radius * radius)) {
                if (random.nextFloat() <= dropChance && !nuke) {
                    ...
                    ACCompat.onBlockExploded(blockstate, level(), blockpos, dummyExplosion);
                }
    ...
    private void scareMobs() {
        ...
        for (LivingEntity e : list) {
            if (!e.getType().builtInRegistryHolder().is(ACTagRegistry.RESISTS_TREMORSAURUS_ROAR) && !isAlliedTo(e)) {
    ...
    *///?} else {
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableMob) {
        return ACCompat.createEntity(ACEntityRegistry.TREMORZILLA.get(), serverLevel);
    }
    ...
    private Stream<BlockPos> getNearbySirens(ServerLevel world, int range) {
        ...
        return pointofinterestmanager.findAll(poiTypeHolder -> poiTypeHolder.is(ACPOIRegistry.NUCLEAR_SIREN_KEY), Predicates.alwaysTrue(), this.blockPosition(), range, PoiManager.Occupancy.ANY);
    }
    ...
    // See SubmarineEntity's copy of this override: a Forge/NeoForge-only IEntityExtension hook with
    // no Fabric counterpart and no caller there.
    //? if !fabric {
    @Override
    ...
    protected float getBlockSpeedFactor() {
        // LivingEntity#onSoulSpeedBlock went away in 1.21 along with the rest of the soul-speed
        // bookkeeping it fronted; the test it performed is the tag check spelled out.
        //? if >=1.21
        /*return this.isTremorzillaSwimming() || this.level().getBlockState(this.getBlockPosBelowThatAffectsMyMovement()).is(net.minecraft.tags.BlockTags.SOUL_SPEED_BLOCKS) ? 1.0F : super.getBlockSpeedFactor();*/
        //? if <1.21
    ...
    *///?} else {
    ...
    // See AtlatitanEntity#getAgeScale — getScale() is final from 1.21.2, getAgeScale() is the hook.
    //? if >=1.21.2
