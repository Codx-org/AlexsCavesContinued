...
public class GumWormEntity extends Monster implements ICustomCollisions, KaijuMob, ShakesScreen, CullingBoundsEntity {
    ...
    private static final EntityDataAccessor<Optional<UUID>> RIDING_SEGMENT_UUID = SynchedEntityData.defineId(GumWormEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, 0.25D).add(ACCompat.attribute(ACPlatform.entityGravityAttribute()), 0.15D).add(Attributes.MAX_HEALTH, 150.0D).add(Attributes.ARMOR, 10.0D).add(Attributes.ATTACK_DAMAGE, 9.0D).add(Attributes.FOLLOW_RANGE, 128.0D);
    }
    ...
    protected float getStandingEyeHeight(Pose pose, EntityDimensions dimensions) {
        return 0.4F * ACCompat.height(dimensions);
    }
    ...
    public void tick() {
        ...
        }else if((surfaceY < this.getEyeY() || centralStateBelow.isAir() || ACFluids.isInAnyFluid(this)) && isSafeDig(level(), this.blockPosition().below()) && !isRidingMode() && !this.isLeaping()){
    ...
    protected void dropAllDeathLoot(DamageSource damageSource) {
        ...
        // Looting stopped being a number a hook could hand out in 1.21 — it is an enchantment
        // effect component applied through the loot context now, and NeoForge dropped
        // getLootingLevel with it. Nothing below still reads `i` on those versions: the two call
        // sites that took it, dropCustomDeathLoot and onLivingDrops, both lost the parameter and
        // are rewritten by the 1.21 replacement rules, so the local is gated away rather than
        // recomputed.
        //? if <1.21
        int i = net.minecraftforge.common.ForgeHooks.getLootingLevel(this, entity, damageSource);
        com.github.alexmodguy.alexscaves.server.misc.ACCompat.captureDrops(this, new java.util.ArrayList<>());

        // Deliberately unqualified: the !mc2105-lasthurtbyplayer rule rewrites the literal
        // "this.lastHurtByPlayer", and a replacement matches in prefix position, so writing
        // "this." here would turn the field name into "this.getLastHurtByPlayer()Time".
        boolean flag = lastHurtByPlayerTime > 0;
        if (this.shouldDropLoot() && ACCompat.gameRule(this.level(), GameRules.RULE_DOMOBLOOT, true)) {
        ...
        Collection<ItemEntity> drops = com.github.alexmodguy.alexscaves.server.misc.ACCompat.captureDrops(this, null);
        // 1.20.5 moved this hook off ForgeHooks and onto ForgeEventFactory. It cannot be a
        // replacement rule: the NeoForge rename group already claims the ForgeHooks token, and the
        // two rules would have to run in a particular order to both land.
        //? if forge && >=1.20.5
        /*if (!net.minecraftforge.event.ForgeEventFactory.onLivingDrops(this, damageSource, drops, i, flag)){*/
        //? if !forge || <1.20.5
        if (!net.minecraftforge.common.ForgeHooks.onLivingDrops(this, damageSource, drops, i, flag)){
        ...
        for (LivingEntity living : level().getEntitiesOfClass(LivingEntity.class, hurtBox, EntitySelector.NO_CREATIVE_OR_SPECTATOR)) {
            if (!living.is(this) && !living.isAlliedTo(this) && !isRidingPlayer(living) && living.getType() != this.getType()) {
                if (ACCompat.hurt(living, damageSource, damageAmount)) {
                    ACCompat.knockback(living, knockbackAmount, this.getX() - living.getX(), this.getZ() - living.getZ());
                }
    ...
    public void spawnDustParticles(boolean surface) {
        if (surfaceY < this.getY(1.0F) && this.getY(1.0F) - surfaceY < 4) {
            ...
            if (!state.isAir()) {
                ACCompat.addParticleForced(level(), new BlockParticleOption(ACParticleRegistry.BIG_BLOCK_DUST.get(), state), this.getRandomX(0.8F), surfaceY + random.nextFloat(), this.getRandomZ(0.8F), (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F);
            }
        }
    }
    ...
    *///?} else {
    ...
    // 1.20.5 dropped finalizeSpawn's trailing spawn-data CompoundTag; EntityType applies it itself now.
    //? if >=1.20.5 {
    ...
    *///?} else {
    ...
    public void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (this.getRidingSegmentUUID() != null) {
            ACCompat.putUUID(compoundTag, "RidingSegmentUUID", this.getRidingSegmentUUID());
        }
    ...
    public void readAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (compoundTag.contains("GobthumperX") && compoundTag.contains("GobthumperY") && compoundTag.contains("GobthumperZ")) {
            this.setGobthumperPos(new BlockPos(ACCompat.getInt(compoundTag, "GobthumperX"), ACCompat.getInt(compoundTag, "GobthumperY"), ACCompat.getInt(compoundTag, "GobthumperZ")));
        }
        if (ACCompat.hasUUID(compoundTag, "RidingSegmentUUID")) {
            this.setRidingSegmentUUID(ACCompat.getUUID(compoundTag, "RidingSegmentUUID"));
        }
        this.setTempSummon(ACCompat.getBoolean(compoundTag, "TempSummon"));
    }
