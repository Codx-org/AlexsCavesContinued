...
public class OccultGemItem extends Item implements ACTickingItem {
    ...
    public static GlobalPos getBeholderPosition(CompoundTag tag) {
        ...
        if (flag && flag1) {
            ...
            if (optional.isPresent()) {
                BlockPos blockpos = ACCompat.getBlockPos(tag, "BeholderPos");
    ...
    public void acInventoryTick(ItemStack itemStack, Level level, Entity entity, boolean b) {
        if (!level.isClientSide()) {
            if (isActive(itemStack)) {
                ...
                if (compoundtag.contains("BeholderTracked") && !ACCompat.getBoolean(compoundtag, "BeholderTracked")) {
                    return;
                }
                ...
                if (optional.isPresent() && optional.get() == level.dimension() && compoundtag.contains("BeholderPos")) {
                    BlockPos blockpos = ACCompat.getBlockPos(compoundtag, "BeholderPos");
                    boolean flag = false;
    ...
    private void addBeholderTags(ResourceKey<Level> levelResourceKey, BlockPos blockPos, CompoundTag tag) {
        ACCompat.putBlockPos(tag, "BeholderPos", blockPos);
    ...
    public boolean isFoil(ItemStack stack) {
        if (ACCompat.getTag(stack) != null && ACCompat.getTag(stack).contains("BeholderPos")) {
            Optional<ResourceKey<Level>> optional = getBeholderDimension(ACCompat.getTag(stack));
            BlockPos blockpos = ACCompat.getBlockPos(ACCompat.getTag(stack), "BeholderPos");
