...
import com.github.alexmodguy.alexscaves.server.misc.ACItemAccess;
...
public class GingerbreadManStealGoal extends Goal {
    ...
    public static boolean hasStealableInventory(Entity entity) {
        ...
        ACItemAccess handler = ACPlatform.entityItemHandler(entity);
        if (handler != null) {
            for (int i = 0; i < handler.size(); i++) {
                if (handler.stackInSlot(i).is(ACTagRegistry.GINGERBREAD_MAN_STEALS)) {
                    return true;
                }
            }
        }
    ...
    public ItemStack stealOneFrom(Entity entity) {
        ...
        if (handler != null) {
            ...
            if (!validSlots.isEmpty()) {
                ...
                // The slot was already matched against the tag when validSlots was built, so the
                // upstream re-test before shrinking is what takeOne's own emptiness check covers.
                return handler.takeOne(slotId);
            }
        }
