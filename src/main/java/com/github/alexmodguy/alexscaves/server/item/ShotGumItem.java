...
public class ShotGumItem extends Item implements UpdatesStackTags, AlwaysCombinableOnAnvil, ACClientExtensionItem, ACEnchantableItem, ACTickingItem {
    ...
    public static float getLerpedShootTime(ItemStack stack, float f) {
        ...
        float prev = compoundtag != null ? (float) ACCompat.getInt(compoundtag, "PrevShootTime") : 0F;
        float current = compoundtag != null ? (float) ACCompat.getInt(compoundtag, "ShootTime") : 0F;
        return prev + f * (current - prev);
    }

    public static float getLerpedCrankAngle(ItemStack stack, float f) {
        CompoundTag compoundtag = ACCompat.getTag(stack);
        float prev = compoundtag != null ? (float) ACCompat.getFloat(compoundtag, "PrevCrankAngle") : 0F;
        float current = compoundtag != null ? (float) ACCompat.getFloat(compoundtag, "CrankAngle") : 0F;
        return prev + f * (current - prev);
    }

    public static int getShootTime(ItemStack stack) {
        CompoundTag compoundtag = ACCompat.getTag(stack);
        return compoundtag != null ? ACCompat.getInt(compoundtag, "ShootTime") : 0;
    }
    ...
    public static float getCrankAngle(ItemStack stack) {
        CompoundTag compoundtag = ACCompat.getTag(stack);
        return compoundtag != null ? ACCompat.getFloat(compoundtag, "CrankAngle") : 0;
    }
    ...
    public static int getGumballsLeft(ItemStack stack) {
        CompoundTag compoundtag = ACCompat.getTag(stack);
        return compoundtag != null && compoundtag.contains("Gumballs") ? ACCompat.getInt(compoundtag, "Gumballs") : MAX_AMMO;
    }
    ...
    public static boolean isShooting(ItemStack stack) {
        CompoundTag compoundtag = ACCompat.getTag(stack);
        return compoundtag != null && ACCompat.getBoolean(compoundtag, "Shooting");
    }
    ...
    public void acInventoryTick(ItemStack stack, Level level, Entity entity, boolean held) {
        ...
        if (ACCompat.getInt(tag, "PrevShootTime") != ACCompat.getInt(tag, "ShootTime")) {
            tag.putInt("PrevShootTime", getShootTime(stack));
        }
        if (ACCompat.getFloat(tag, "PrevCrankAngle") != ACCompat.getFloat(tag, "CrankAngle")) {
        ...
        if(shootTime == 5 && shooting){
            ...
            boolean ricochetEnchant = ACCompat.enchantLevel(stack, ACEnchantmentRegistry.TARGETED_RICOCHET) > 0;
            boolean splitEnchant = ACCompat.enchantLevel(stack, ACEnchantmentRegistry.TRIPLE_SPLIT) > 0;
            boolean explosiveEnchant = ACCompat.enchantLevel(stack, ACEnchantmentRegistry.EXPLOSIVE_FLAVOR) > 0;
            int maximumBounces = 4 + ACCompat.enchantLevel(stack, ACEnchantmentRegistry.BOUNCY_BALL) * 2;
