...
public class CandyCaneHookEntity extends ThrowableProjectile {
    ...
    private static final EntityDataAccessor<Optional<UUID>> HOOKED_ENTITY_UUID = SynchedEntityData.defineId(CandyCaneHookEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public CandyCaneHookEntity(Player player, Level level, ItemStack itemstack, boolean offhand) {
        ...
        ACCompat.moveTo(this, d0 + armOffset.x, d1, d2 + armOffset.z, f1, f);
        Vec3 vec3 = new Vec3(-f3, Mth.clamp(-(f5 / f4), -5.0F, 5.0F), -f2);
        double d3 = vec3.length();
        double launchDist = 0.5D + ACCompat.enchantLevel(itemstack, ACEnchantmentRegistry.FAR_FLUNG) * 0.2D;
        ...
        this.setDamage(ACCompat.enchantLevel(itemstack, ACEnchantmentRegistry.SHARP_CANE) * 3.0F);
        if(ACCompat.enchantLevel(itemstack, ACEnchantmentRegistry.STRAIGHT_HOOK) > 0){
    ...
    *///?} else {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(REELING, false);
        this.entityData.define(OFFHAND, false);
        this.entityData.define(HOOKED_POSITION, Optional.empty());
        this.entityData.define(HOOKED_ENTITY_ID, -1);
        this.entityData.define(HOOKED_ENTITY_UUID, Optional.empty());
        this.entityData.define(DAMAGE, 0.0F);
        this.entityData.define(RESISTS_GRAVITY, false);
    }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        this.setReeling(ACCompat.getBoolean(tag, "Reeling"));
        if(tag.contains("HookedEntityUUID")){
            this.setHookedEntityUUID(ACCompat.getUUID(tag, "HookedEntityUUID"));
        }
        this.setOffhand(ACCompat.getBoolean(tag, "Offhand"));
    }
    ...
    public void addAdditionalSaveData(CompoundTag tag) {
        ...
        if(this.getHookedEntityUUID() != null){
            ACCompat.putUUID(tag, "HookedEntityUUID", this.getHookedEntityUUID());
        }
