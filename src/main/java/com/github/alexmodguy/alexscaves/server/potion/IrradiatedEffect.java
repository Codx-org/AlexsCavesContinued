...
public class IrradiatedEffect extends ACMobEffect {
    ...
    public void tick(LivingEntity entity, int tick) {
        ...
        if (!(entity instanceof RaycatEntity) && entity.level().getRandom().nextFloat() < damageScale + 0.1F) {
            entity.hurt(ACDamageTypes.causeRadiationDamage(entity.level().registryAccess()), damageScale);
        }
    }

    @Override
    public boolean shouldTick(int tick1, int level) {
        if (level <= 0) {
            return false;
        }
        int j = 200 / level;
