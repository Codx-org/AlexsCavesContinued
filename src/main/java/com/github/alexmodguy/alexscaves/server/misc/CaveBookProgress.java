...
public class CaveBookProgress {
    ...
    private CaveBookProgress(CompoundTag tag) {
        if (tag.contains("Pages")) {
            ListTag listTag = ACCompat.getList(tag, "Pages", 10);
            for (int i = 0; i < listTag.size(); ++i) {
                CompoundTag compoundtag = ACCompat.getCompound(listTag, i);
                unlockedPages.put(ACCompat.getString(compoundtag, "Category"), Subcategory.getByOrdinal(ACCompat.getInt(compoundtag, "SubCategory")));
            }
        }
    }
    ...
    public static CaveBookProgress getCaveBookProgress(Player player) {
        ...
        CompoundTag tag1 = tag.contains(PLAYER_CAVE_BOOK_PROGRESS_TAG) ? ACCompat.getCompound(tag, PLAYER_CAVE_BOOK_PROGRESS_TAG) : new CompoundTag();
