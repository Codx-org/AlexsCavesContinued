...
public class VolcanicCoreBlockEntity extends BlockEntity {
    ...
    public static void tick(Level level, BlockPos blockPos, BlockState state, VolcanicCoreBlockEntity entity) {
        ...
        if (AlexsCaves.PROXY.isPrimordialBossActive(level)) {
            if (entity.tephraSpawnCooldown-- <= 0) {
                ...
                entity.tephraSpawnCooldown = 120 + level.getRandom().nextInt(120);
            }
        } else {
            entity.battleTime = 0;
        }
    ...
    public void load(CompoundTag tag) {
        super.load(tag);
        this.tephraSpawnCooldown = ACCompat.getInt(tag, "TephraSpawnCooldown");
        this.bossSpawnCooldown = ACCompat.getInt(tag, "BossSpawnCooldown");
    }
    ...
    public boolean spawnBoss(){
        if(bossSpawnCooldown > 0){
            ...
            LuxtructosaurusEntity luxtructosaurus = ACCompat.createEntity(ACEntityRegistry.LUXTRUCTOSAURUS.get(), level);
    ...
    private void spawnTephra(boolean big) {
        ...
        if (level.getBlockState(volcanoTop).is(ACBlockRegistry.VOLCANIC_CORE.get()) || level.getBlockState(volcanoTop).is(ACBlockRegistry.PRIMAL_MAGMA.get()) || level.getBlockState(volcanoTop).is(ACBlockRegistry.FISSURE_PRIMAL_MAGMA.get()) || level.getFluidState(volcanoTop).is(FluidTags.LAVA)) {
            ...
            if (big) {
                TephraEntity bigTephra = ACCompat.createEntity(ACEntityRegistry.TEPHRA.get(), level);
                bigTephra.setPos(volcanoVec);
                bigTephra.setMaxScale(2F + level.getRandom().nextFloat());
                Vec3 targetVec;
                if (nearestPlayer == null) {
                    targetVec = new Vec3(level.getRandom().nextFloat() - 0.5F, 0, level.getRandom().nextFloat() - 0.5F).normalize().scale(level.getRandom().nextInt(50) + 20);
                } else {
                ...
                double d5 = nearestPlayer == null ? level.getRandom().nextFloat() : 0;
                bigTephra.shoot(targetVec.x, targetVec.y + 0.5F + d4 * 0.75F + d5, targetVec.z,  (float) (d4 * 0.1F + d5), 1 + level.getRandom().nextFloat() * 0.5F);
                level.addFreshEntity(bigTephra);
            }
            for(int smalls = 0; smalls < 3 + level.getRandom().nextInt(3); smalls++){
                TephraEntity smallTephra = ACCompat.createEntity(ACEntityRegistry.TEPHRA.get(), level);
                smallTephra.setPos(volcanoVec);
                smallTephra.setMaxScale(0.6F + 0.6F * level.getRandom().nextFloat());
                Vec3 targetVec = new Vec3(level.getRandom().nextFloat() - 0.5F, 0, level.getRandom().nextFloat() - 0.5F).normalize().scale(level.getRandom().nextInt(30) + 30);
                double d4 = Math.sqrt(targetVec.x * targetVec.x + targetVec.z * targetVec.z);
                smallTephra.shoot(targetVec.x, targetVec.y + 0.5F + d4 * 0.75F + level.getRandom().nextFloat(), targetVec.z, (float) (d4 * 0.1F + level.getRandom().nextFloat()), 1);
