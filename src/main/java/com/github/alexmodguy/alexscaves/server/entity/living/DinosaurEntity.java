...
public abstract class DinosaurEntity extends TamableAnimal implements IDancesToJukebox, LaysEggs, IAdvancedPathingMob, RidingMeterMount {
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setHasEgg(ACCompat.getBoolean(compound, "Egg"));
        int altSkin = ACCompat.getInt(compound, "AltSkin");
        //compatibility with pre 1.1.0 saves
        if (compound.contains("Retro") && ACCompat.getBoolean(compound, "Retro")) {
            altSkin = 1;
        }
    ...
    public void handleEntityEvent(byte b) {
        if (b == 77) {
            ...
            for (int i1 = 0; i1 < particleCount; i1++) {
                ...
                if (groundState.isSolid()) {
                    if (level().isClientSide()) {
                        ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, groundState), this.getX() + extraX, ground.getY(), this.getZ() + extraZ, motionX, motionY, motionZ);
                    }
                }
            }
        } else if (b == 78) {
            ...
            for (int i = 0; i < 15 + level().getRandom().nextInt(5); i++) {
            ...
            if (altSkinFromItem > 0) {
                ...
                if (canOwnerCommand(player)) {
                    ...
                    if (this.getType() == ACEntityRegistry.SUBTERRANODON.get() && this.canAddPassenger(player)) {
                        ACCompat.moveTo(this, this.getX(), this.getY() + player.getBbHeight() + 0.5F, this.getZ());
                    }
    ...
    // See DeepOneBaseEntity — isAlliedTo(Entity) is final from 1.21.2 and considersEntityAsAlly is
    // the hook. The owner call two lines down is a *call site* on another entity, so it keeps the
    // public spelling; only this method's own header and its super call move.
    @Override
    //? if >=1.21.2
