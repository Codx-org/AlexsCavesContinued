...
public class GloomothEntity extends PathfinderMob implements UnderzealotSacrifice, CullingBoundsEntity {
    ...
    public void tick() {
        ...
        if (isBeingSacrificed && !level().isClientSide()) {
            ...
            if (sacrificeTime < 0) {
                ...
                this.stopRiding();
                WatcherEntity watcherEntity = ACCompat.convertTo(this, ACEntityRegistry.WATCHER.get(), true);
    ...
    public BlockPos getNearestMothBall(ServerLevel world, BlockPos to, int range) {
        ...
        return pointofinterestmanager.findClosest(poiTypeHolder -> poiTypeHolder.is(ACPOIRegistry.MOTH_BALL_KEY), blockPos -> isWithinMothBallRange(blockPos, range), to, range, PoiManager.Occupancy.ANY).orElse(null);
    }
