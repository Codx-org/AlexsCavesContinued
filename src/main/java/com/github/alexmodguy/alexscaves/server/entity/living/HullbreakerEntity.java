...
public class HullbreakerEntity extends WaterAnimal implements IAnimatedEntity, KaijuMob, ACMultipartOwner {
    ...
    public static final Predicate<LivingEntity> GLOWING_TARGET = (mob) -> {
        return ACCompat.isInWaterOrBubble(mob) && (mob.hasEffect(MobEffects.GLOWING) || mob.getType().builtInRegistryHolder().is(ACTagRegistry.GLOWING_ENTITIES) || mob.isPassenger() && mob.getVehicle() instanceof SubmarineEntity sub && sub.areLightsOn());
    };
    ...
    public ItemEntity spawnAtLocation(ItemStack stack) {
        ...
        if (itementity != null) {
            if(this.headPart != null){
                ...
                itementity.setDeltaMovement(yOnlyViewVector.add(random.nextFloat() * 0.2F - 0.1F, random.nextFloat() * 0.2F - 0.1F, random.nextFloat() * 0.2F - 0.1F).normalize().scale(0.8F + level().getRandom().nextFloat() * 0.3F));
            }
    ...
    protected void tickDeath() {
        ...
        if(!level().isClientSide()){
            ...
            if(this.getAnimation() == ANIMATION_DIE && this.getAnimationTick() > 10 && this.getAnimationTick() % 7 == 0 && collectedLoot && !deathItems.isEmpty()){
                ...
                ACCompat.spawnAtLocation(this, randomItem.copy());
    ...
    private void populateDeathLootForHullbreaker(){
        ResourceLocation resourcelocation = ACCompat.lootId(this.getLootTable());
        // From 1.21.2 that can be absent — see ACCompat#lootId — and "no table" means no drops.
    ...
    protected float getStandingEyeHeight(Pose pose, EntityDimensions dimensions) {
        return 0.45F * ACCompat.height(dimensions);
    }
    ...
    public void tick() {
        ...
        boolean grounded = this.onGround() && !ACCompat.isInWaterOrBubble(this);
        ...
        pulseAmount += pulseBy;
        if (!level().isClientSide()) {
            double waterHeight = ACFluids.waterHeight(this);
    ...
    public void breakBlock() {
        ...
        if (!level().isClientSide() && com.github.alexmodguy.alexscaves.server.misc.ACPlatform.canEntityGrief(level(), this) && this.getTarget() instanceof Player) {
    ...
    protected SoundEvent getAmbientSound() {
        return ACCompat.isInWaterOrBubble(this) ? ACSoundRegistry.HULLBREAKER_IDLE.get() : ACSoundRegistry.HULLBREAKER_LAND_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return ACCompat.isInWaterOrBubble(this) ? ACSoundRegistry.HULLBREAKER_HURT.get() : ACSoundRegistry.HULLBREAKER_LAND_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return ACCompat.isInWaterOrBubble(this) ? ACSoundRegistry.HULLBREAKER_DEATH.get() : ACSoundRegistry.HULLBREAKER_LAND_DEATH.get();
    }
