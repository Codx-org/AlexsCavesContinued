...
public class GobthumperBlockEntity extends BlockEntity {
        ...
        if(level.isClientSide()){
            ...
            if(this.summonedWormId != -1 && level.getEntity(this.summonedWormId) instanceof GumWormEntity gumWorm && gumWorm.isAlive()){
                ...
                if (closestWorm != null) {
                    ...
                    if(flag){
                        GumWormEntity summonedWorm = ACCompat.createEntity(ACEntityRegistry.GUM_WORM.get(), level);
                        summonedWorm.setPos(Vec3.atCenterOf(summonPos));
                        summonedWorm.setTempSummon(true);
                        ACCompat.finalizeSpawn(summonedWorm, (ServerLevel) level, ((ServerLevel) level).getCurrentDifficultyAt(summonPos), MobSpawnType.MOB_SUMMONED, (SpawnGroupData) null);
    ...
    public void load(CompoundTag tag) {
        super.load(tag);
        this.summonedWormId = ACCompat.getInt(tag, "SummonedWormID");
    }
