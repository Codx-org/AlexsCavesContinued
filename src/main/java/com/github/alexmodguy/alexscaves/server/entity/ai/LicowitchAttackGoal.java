...
public class LicowitchAttackGoal extends Goal {
    ...
    private void potionAttack(LivingEntity target) {
        ...
        // 1.21.2 folded the thrown stack into the constructor — same reason as every other
        // ThrowableItemProjectile — so the potion is named up front instead of being set after.
        // The setItem below is then a no-op restating it, and stays because it is what the older
        // nodes do; both paths end up throwing exactly LicowitchEntity.hungerPotion().
        //? if >=1.21.2 {
        /*ThrownPotion thrownpotion = new ThrownPotion(licowitch.level(), licowitch, LicowitchEntity.hungerPotion());
        *///?} else {
        ...
        if (summonSpace && licowitch.getRandom().nextBoolean()) {
            ...
            if (summonSpot != null) {
                ...
                if (licowitch.level() instanceof ServerLevel serverLevel) {
                    ACCompat.finalizeSpawn(summoned, serverLevel, serverLevel.getCurrentDifficultyAt(BlockPos.containing(summonSpot)), MobSpawnType.MOB_SUMMONED, null);
                }
                ...
                flag = true;
                licowitch.level().playSound((Player) null, licowitch.blockPosition(), ACSoundRegistry.LICOWITCH_CAST_SUMMON.get(), SoundSource.HOSTILE, 0.3F, 0.9F + licowitch.level().getRandom().nextFloat() * 0.2F);
            }
        }
        if (!flag) {
            ...
            SugarStaffHexEntity sugarStaffHexEntity = ACCompat.createEntity(ACEntityRegistry.SUGAR_STAFF_HEX.get(), licowitch.level());
            ...
            licowitch.level().playSound((Player) null, licowitch.blockPosition(), ACSoundRegistry.LICOWITCH_CAST_HEX.get(), SoundSource.HOSTILE, 0.3F, 0.9F + licowitch.level().getRandom().nextFloat() * 0.2F);
        }
    }
    ...
    private void peppermintAttack(LivingEntity target) {
        ...
        for (int i = 0; i < spawnIn; i++) {
            SpinningPeppermintEntity spinningPeppermintEntity = ACCompat.createEntity(ACEntityRegistry.SPINNING_PEPPERMINT.get(), licowitch.level());
        ...
        licowitch.level().playSound((Player) null, licowitch.blockPosition(), ACSoundRegistry.LICOWITCH_CAST_PEPPERMINT.get(), SoundSource.HOSTILE, 0.3F, 0.9F + licowitch.level().getRandom().nextFloat() * 0.2F);
    }
}