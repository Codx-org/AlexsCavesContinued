...
public class NuclearFurnaceMenu extends AbstractContainerMenu {
    ...
    private boolean canSmelt(ItemStack stack) {
        // See NuclearFurnaceBlockEntity#quickCheck: 1.21 asks with a typed RecipeInput, and 1.21.2
        // took the recipe book off the client entirely — vanilla answers this from a synced
        // RecipePropertySet, which a custom recipe type has none of. Shift-clicking is resolved
        // server-side anyway, so the client optimistically says yes and the container update that
        // follows the click corrects it.
        //? if >=1.21.2
        /*return !(this.level instanceof net.minecraft.server.level.ServerLevel serverLevel) || serverLevel.recipeAccess().getRecipeFor(NuclearFurnaceBlockEntity.getRecipeType(), new net.minecraft.world.item.crafting.SingleRecipeInput(stack), this.level).isPresent();*/
        //? if >=1.21 && <1.21.2
        /*return this.level.getRecipeManager().getRecipeFor(NuclearFurnaceBlockEntity.getRecipeType(), new net.minecraft.world.item.crafting.SingleRecipeInput(stack), this.level).isPresent();*/
        //? if <1.21
