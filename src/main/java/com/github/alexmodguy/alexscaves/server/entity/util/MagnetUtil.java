...
public class MagnetUtil {
    ...
    private static Stream<BlockPos> getNearbyAttractingMagnets(BlockPos blockpos, ServerLevel world, int range) {
        ...
        return pointofinterestmanager.findAll(poiTypeHolder -> poiTypeHolder.is(ACPOIRegistry.ATTRACTING_MAGNETS_KEY), Predicates.alwaysTrue(), blockpos, range, PoiManager.Occupancy.ANY);
    }

    private static Stream<BlockPos> getNearbyRepellingMagnets(BlockPos blockpos, ServerLevel world, int range) {
        PoiManager pointofinterestmanager = world.getPoiManager();
        return pointofinterestmanager.findAll(poiTypeHolder -> poiTypeHolder.is(ACPOIRegistry.REPELLING_MAGNETS_KEY), Predicates.alwaysTrue(), blockpos, range, PoiManager.Occupancy.ANY);
    }
    ...
    private static boolean isDynamicallyMagnetic(LivingEntity entity, boolean legsOnly) {
        if (entity.hasEffect(ACCompat.effect(ACEffectRegistry.MAGNETIZING.get()))) {
            return true;
        } else if (legsOnly) {
    ...
    public static boolean isPulledByMagnets(Entity entity) {
        ...
        return entity.getType().builtInRegistryHolder().is(ACTagRegistry.MAGNETIC_ENTITIES);
    }
    ...
    public static AABB rotateBoundingBox(EntityDimensions dimensions, Direction dir, Vec3 position) {
        float w = ACCompat.width(dimensions);
        float h = ACCompat.height(dimensions);
        float usualWidth = w * 0.5F;
        switch (dir) {
            case NORTH:
                return new AABB(w * -0.5F, w * -0.5F, -usualWidth, w * 0.5F, w * 0.5F, h - usualWidth).move(position);
            case SOUTH:
                return new AABB(w * -0.5F, w * -0.5F, -h + usualWidth, w * 0.5F, w * 0.5F, usualWidth).move(position);
            case EAST:
                return new AABB(-h + usualWidth, w * -0.5F, w * -0.5F, usualWidth, w * 0.5F, w * 0.5F).move(position);
            case WEST:
                return new AABB(-usualWidth, w * -0.5F, w * -0.5F, h - usualWidth, w * 0.5F, w * 0.5F).move(position);
        }
