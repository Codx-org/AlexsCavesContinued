...
public class TremorzillaPartEntity extends PartEntity<TremorzillaEntity> implements CullingBoundsEntity {
    ...
    // See SauropodPartEntity#isInvulnerableTo — same split, same reason for hand-gating it.
    //? if >=1.21.2 {
    /*public boolean isInvulnerableTo(net.minecraft.world.damagesource.DamageSource damageSource) {
        return this.isInvulnerableToBase(damageSource) || damageSource.is(ACDamageTypes.ACID) || damageSource.getEntity() != null && this.getParent().isPassengerOfSameVehicle(damageSource.getEntity());
    }
    *///?} else {
