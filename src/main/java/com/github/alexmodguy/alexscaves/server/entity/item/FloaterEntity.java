    ...
    *///?}
    ...
    public void handleEntityEvent(byte message) {
        if (message == 3) {
            for (int i = 0; i < 10 + random.nextInt(4); ++i) {
                this.level().addParticle(ACCompat.itemParticle(ParticleTypes.ITEM, new ItemStack(ACItemRegistry.FLOATER.get())), this.getRandomX(1.0D), this.getRandomY(), this.getRandomZ(1.0D), ((double) this.random.nextFloat() - 0.5D) * 0.3D, ((double) this.random.nextFloat() - 0.5D) * 0.3D, ((double) this.random.nextFloat() - 0.5D) * 0.3D);
            }
        } else {
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger) && passenger instanceof LivingEntity living && !this.touchingUnloadedChunk()) {
            double d0 = this.getY() + 0.7F + ACPlatform.myRidingOffset(passenger, this);
