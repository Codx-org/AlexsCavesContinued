...
public class ExtinctionSpearItem extends SpearItem implements ACEnchantableItem, ACRepairableItem {
    ...
    public void releaseUsing(ItemStack itemStack, Level level, LivingEntity livingEntity, int i1) {
        if (livingEntity instanceof Player player) {
            ...
            if (f > 0.1D) {
                ACCompat.hurtAndBreak(itemStack, 1, livingEntity, EquipmentSlot.MAINHAND);
    ...
    public void onUseTick(Level level, LivingEntity living, ItemStack stack, int timeUsing) {
        if(timeUsing == getUseDuration(stack)){
            ...
            int grottoHeads = 3 + ACCompat.enchantLevel(stack, ACEnchantmentRegistry.HERD_PHALANX);
            float grottoRotateBy = 360F / grottoHeads;
            for(int i = 0; i < grottoHeads; i++){
                DinosaurSpiritEntity dinosaurSpirit = ACCompat.createEntity(ACEntityRegistry.DINOSAUR_SPIRIT.get(), level);
    ...
    protected boolean acHurtEnemy(ItemStack stack, LivingEntity hurtEntity, LivingEntity player) {
        ...
        DinosaurSpiritEntity dinosaurSpirit = ACCompat.createEntity(ACEntityRegistry.DINOSAUR_SPIRIT.get(), player.level());
        ...
        dinosaurSpirit.setEnchantmentLevel(ACCompat.enchantLevel(stack, ACEnchantmentRegistry.CHOMPING_SPIRIT));
        ...
        return super.acHurtEnemy(stack, hurtEntity, player);
    }
    ...
    public Item[] acExtraRepairMaterials() {
        return new Item[]{ACItemRegistry.TECTONIC_SHARD.get()};
    }
