...
public class MineGuardianEntity extends Monster {
    ...
    private static final EntityDataAccessor<Optional<UUID>> ANCHOR_UUID = SynchedEntityData.defineId(MineGuardianEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    protected SoundEvent getAmbientSound() {
        return ACCompat.isInWaterOrBubble(this) ? ACSoundRegistry.MINE_GUARDIAN_IDLE.get() : ACSoundRegistry.MINE_GUARDIAN_LAND_IDLE.get();
    }

    protected SoundEvent getHurtSound(DamageSource damageSource) {
        return ACCompat.isInWaterOrBubble(this) ? ACSoundRegistry.MINE_GUARDIAN_HURT.get() : ACSoundRegistry.MINE_GUARDIAN_LAND_HURT.get();
    }

    protected SoundEvent getDeathSound() {
        return ACCompat.isInWaterOrBubble(this) ? ACSoundRegistry.MINE_GUARDIAN_DEATH.get() : ACSoundRegistry.MINE_GUARDIAN_LAND_DEATH.get();
    }
    ...
    public void tick() {
        ...
        if (this.isExploding()) {
            if (explodeProgress >= 10.0F) {
                ...
                MineExplosion explosion = new MineExplosion(level(), this, this.getX(), this.getY(0.5), this.getZ(), 5.0F, ACCompat.isInWaterOrBubble(this), blockinteraction);
                explosion.explode();
        ...
        if (!level().isClientSide()) {
            ...
            if (this.isScanning()) {
                ...
                if (timeSinceHadTarget == 0 || !ACCompat.isInWaterOrBubble(this)) {
                    this.setEyeClosed(false);
                } else if (ACCompat.isInWaterOrBubble(this) && (timeSinceHadTarget > maxSleepTime && j > 200 || this.hurtTime > 0)) {
                ...
                if (ACCompat.isInWaterOrBubble(this) && timeSinceHadTarget > 100) {
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (ACCompat.hasUUID(compound, "AnchorUUID")) {
            this.setAnchorUUID(ACCompat.getUUID(compound, "AnchorUUID"));
        }
        this.setMaxChainLength(ACCompat.getInt(compound, "MaxChainLength"));
        this.scanTime = ACCompat.getInt(compound, "ScanTime");
        this.setEyeClosed(ACCompat.getBoolean(compound, "EyeClosed"));
    }

    public void addAdditionalSaveData(CompoundTag compound) {
        super.addAdditionalSaveData(compound);
        if (this.getAnchorUUID() != null) {
            ACCompat.putUUID(compound, "AnchorUUID", this.getAnchorUUID());
        }
    ...
    private class MeleeGoal extends Goal {
        ...
        public void tick() {
            ...
            if (target != null) {
                ...
                if (dist > 2.0F) {
                    if (ACCompat.isInWaterOrBubble(MineGuardianEntity.this)) {
