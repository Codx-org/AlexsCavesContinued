...
public class AbyssalAltarBlockEntity extends BaseContainerBlockEntity implements WorldlyContainer, ACUpdatePacketReceiver {
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, AbyssalAltarBlockEntity entity) {
        if (level.isClientSide()) {
            ...
            // Forge patches ItemStack#equals(ItemStack, boolean); the vanilla spelling of the
            // `false` (compare tags too) case is the static matches().
        ...
        if (entity.popStack != null && !level.isClientSide()) {
            if (entity.popDelay++ > 6) {
                ...
                if (entity.lastInteracter != null) {
                    ACPlatform.setThrower(itemEntity, entity.lastInteracter);
                    boolean kill = true;
    ...
    public AABB getRenderBoundingBox() {
        return ACPlatform.encapsulating(worldPosition, worldPosition.offset(1, 2, 1));
    }
    ...
    public void setItem(int index, ItemStack stack) {
        boolean flag = !stack.isEmpty() && ACCompat.sameItemSameData(stack, this.stacks.get(index));
    ...
    public void load(CompoundTag compound) {
        ...
        if (compound.contains("PopStack")) {
            this.popStack = ACCompat.loadStack(ACCompat.BE_REGISTRIES, ACCompat.getCompound(compound, "PopStack"));
        }
        if (ACCompat.hasUUID(compound, "PlayerUUID")) {
            placingPlayer = ACCompat.getUUID(compound, "PlayerUUID");
        }
        slideProgress = ACCompat.getFloat(compound, "SlideAmount");
        prevSlideProgress = ACCompat.getFloat(compound, "PrevSlideAmount");

        itemAngle = ACCompat.getFloat(compound, "Angle");
    }
    ...
    public void saveAdditional(CompoundTag compound) {
        ...
        if (this.popStack != null && !this.popStack.isEmpty()) {
            compound.put("PopStack", ACCompat.saveStack(ACCompat.BE_REGISTRIES, this.popStack));
        }
        if (this.placingPlayer != null) {
            ACCompat.putUUID(compound, "PlayerUUID", this.placingPlayer);
        }
    ...
    public void onDataPacket(Connection net, ClientboundBlockEntityDataPacket packet) {
        if (packet != null && packet.getTag() != null) {
            ...
            this.itemAngle = ACCompat.getFloat(packet.getTag(), "Angle");
        }
    }
    ...
    // 1.20.5 moved getItems/setItems up from RandomizableContainerBlockEntity onto
    // BaseContainerBlockEntity and made them abstract, because that is how the block entity's
    // contents now round-trip through the CONTAINER item component. This class already keeps its
    // own backing list; these just hand it over.
    //? if >=1.20.5 {
    /*@Override
