    ...
    public void releaseUsing(ItemStack itemStack, Level level, LivingEntity living, int time) {
        if (living instanceof Player player) {
            ...
            for(int j = 0; j < Math.ceil(distance * 3); j++){
                Vec3 vec32 = vec31.add(level.getRandom().nextFloat() - 0.5F, level.getRandom().nextFloat() - 0.5F, level.getRandom().nextFloat() - 0.5F);
                Vec3 vec34 = player.getEyePosition().add(level.getRandom().nextFloat() - 0.5F, level.getRandom().nextFloat() - 0.5F, level.getRandom().nextFloat() - 0.5F);
