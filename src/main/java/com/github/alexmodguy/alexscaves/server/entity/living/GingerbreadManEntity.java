...
public class GingerbreadManEntity extends Monster implements IAnimatedEntity, IDancesToJukebox, PossessedByLicowitch, TargetsDroppedItems {
    ...
    private static final EntityDataAccessor<Optional<UUID>> FLEEING_FROM_UUID = SynchedEntityData.defineId(GingerbreadManEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public GingerbreadManEntity(EntityType<? extends Monster> entityType, Level level) {
        ...
        if(this.getNavigation() instanceof GroundPathNavigation groundPathNavigation){
            ...
            groundPathNavigation.getNodeEvaluator().setCanPassDoors(true);
        }
    }
    ...
    public void tick() {
        ...
        if(this.isOvenSpawned()){
            if(despawnFromOvenCooldown-- < 0){
                ACCompat.kill(this);
                ACCompat.spawnAtLocation(this, this.getItemInHand(InteractionHand.MAIN_HAND));
                this.setItemInHand(InteractionHand.MAIN_HAND, ItemStack.EMPTY);
                ACCompat.spawnAtLocation(this, this.getItemInHand(InteractionHand.OFF_HAND));
    ...
    *///?} else {
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setCarryingItem(ACCompat.getBoolean(compound, "CarryingItem"));
        this.setOvenSpawned(ACCompat.getBoolean(compound, "OvenSpawned"));
        this.setGingerbreadTeamColor(ACCompat.getInt(compound, "GingerbreadTeamColor"));
        this.despawnFromOvenCooldown = ACCompat.getInt(compound, "OvenDespawnCooldown");
        this.setLostLimb(true, false, ACCompat.getBoolean(compound, "LostLeftLeg"));
        this.setLostLimb(false, false, ACCompat.getBoolean(compound, "LostRightLeg"));
        this.setLostLimb(true, true, ACCompat.getBoolean(compound, "LostLeftArm"));
        this.setLostLimb(false, true, ACCompat.getBoolean(compound, "LostRightArm"));
    }
    ...
    private void onLoseArm(boolean left, boolean arm) {
        ...
        ItemParticleOption itemParticleOption = ACCompat.itemParticle(ParticleTypes.ITEM, new ItemStack(ACItemRegistry.GINGERBREAD_CRUMBS.get()));
        for(int i = 0; i < 5; i++){
        ...
        if(!level().isClientSide()){
            if(!isOvenSpawned() && shouldDropLoot() && random.nextInt(2) == 0 && this.isAlive()){
                ACCompat.spawnAtLocation(this, new ItemStack(ACItemRegistry.GINGERBREAD_CRUMBS.get()));
            }
            if(arm){
    ...
    private class SitGoal extends Goal {
        ...
        public boolean canUse() {
            if (ACCompat.isInWaterOrBubble(GingerbreadManEntity.this)) {
                return false;
            } else {
                return GingerbreadManEntity.this.isMovementBlocked();
            }
