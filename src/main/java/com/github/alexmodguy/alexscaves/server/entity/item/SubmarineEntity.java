...
public class SubmarineEntity extends Entity implements KeybindUsingMount {
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        this.setOxidizationLevel(ACCompat.getInt(tag, "Oxidization"));
        this.setDamageLevel(ACCompat.getInt(tag, "DamageLevel"));
        this.setWaxed(ACCompat.getBoolean(tag, "Waxed"));
        this.setLightsOn(ACCompat.getBoolean(tag, "LightsOn"));
        if (tag.contains("OxidizeTime")) {
            this.oxidizeTime = ACCompat.getInt(tag, "OxidizeTime");
        }
        if (tag.contains("DamageSustained")) {
            this.damageSustained = ACCompat.getInt(tag, "DamageSustained");
        }
    }
    ...
    public void tick() {
        ...
        if (this.level().isClientSide()) {
            ...
            if (this.isVehicle() && ACCompat.isInWaterOrBubble(this) && this.isAlive()) {
            ...
            if (ACCompat.isInWaterOrBubble(this)) {
    ...
    protected void addPassenger(Entity passenger) {
        ...
        if (this.isControlledByLocalInstance() && this.lSteps > 0) {
            this.lSteps = 0;
            ACCompat.absMoveTo(this, this.lx, this.ly, this.lz, (float) this.lyr, (float) this.lxr);
        }
    }
    ...
    *///?} else {
    ...
    // A Forge/NeoForge IEntityExtension hook: "may a rider stay mounted while this vehicle is
    // submerged in that fluid". Fabric has no counterpart and no caller, so the whole override is
    // gated out there rather than stubbed — a stub would be a method nothing ever invokes, and the
    // answer it gives (vanilla lets a rider stay mounted) is already what happens without it.
    //? if !fabric {
    @Override
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger) && passenger instanceof LivingEntity living && !this.touchingUnloadedChunk()) {
            ...
            double d0 = this.getY() + this.getBbHeight() * 0.5F + seatOffset.y + ACPlatform.myRidingOffset(passenger, this);
    ...
    public void handleEntityEvent(byte b) {
        if (b == 45) {
            ...
            // 26.2 collapsed the four weathering stages of a copper block into one
            // WeatheringCopperCollection<Block>, so Blocks.COPPER_BLOCK is no longer a Block at all
            // and each stage is reached through .weathering(). Same four blocks, same order.
            //? if <26.2 {
            ...
            //?} else {
            /*var copper = Blocks.COPPER_BLOCK.weathering();
            Block particleState = switch (this.getOxidizationLevel()) {
                case 1 -> copper.exposed();
                case 2 -> copper.weathered();
                case 3 -> copper.oxidized();
                default -> copper.unaffected();
            };
            *///?}
            for (int i = 0; i < 10; i++) {
    ...
    public float getWaterHeight() {
        return (float) ACFluids.waterHeight(this);
    }
    ...
    // 1.21.5 tells the entity which column it is standing over, so a bubble column can push from a
    // block that is no longer directly below. The submarine only asks whether it is being ridden,
    // so the position is forwarded untouched.
    //? if >=1.21.5 {
    /*@Override
    public void onAboveBubbleColumn(boolean b, net.minecraft.core.BlockPos acPos) {
        if (!isVehicle()) {
            super.onAboveBubbleColumn(b, acPos);
        }
    }
    ...
    *///?} else {
    ...
    // anything here, and the super call becomes isInvulnerableToBase.
    //? if >=1.21.2 {
    /*public boolean isInvulnerableTo(net.minecraft.world.damagesource.DamageSource damageSource) {
        return this.isInvulnerableToBase(damageSource) || damageSource.is(DamageTypes.DROWN) || damageSource.is(DamageTypes.DRY_OUT) || damageSource.is(DamageTypes.CACTUS) || damageSource.is(DamageTypes.HOT_FLOOR) || damageSource.is(DamageTypes.IN_FIRE) || damageSource.is(DamageTypes.ON_FIRE) || damageSource.is(DamageTypes.FALL);
    }
    *///?} else {
                ...
                if (this.getDamageLevel() >= 4) {
                    if (!this.isRemoved()) {
                        for (int i = 0; i < 2 + random.nextInt(3); i++) {
                            ACCompat.spawnAtLocation(this, Items.COPPER_INGOT);
                        }
                    }
