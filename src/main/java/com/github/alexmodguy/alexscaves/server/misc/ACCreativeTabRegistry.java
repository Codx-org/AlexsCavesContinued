...
public class ACCreativeTabRegistry {
    ...
    // withTabsBefore wants a ResourceKey, which a Supplier handle cannot give. The key is the
    // registration name, so spelling it is exact — and unlike a handle it can be referenced from
    // the initialiser of a tab declared earlier in the file.
    //
    // Both withTabsBefore and the no-argument CreativeModeTab.builder() are loader additions:
    // vanilla's Builder takes the row and column a *vanilla* tab occupies, which a mod tab never
    // knows, and vanilla has no ordering call at all. Fabric API supplies the builder
    // (FabricItemGroup.builder(), through the !fab-creativetab-builder rename) and orders tabs by
    // registration order — which is already the order the withTabsBefore chain below spells out,
    // so those eleven lines are simply gated off there. That leaves this helper unused on Fabric.
    private static ResourceKey<CreativeModeTab> tabKey(String name) {
        return ResourceKey.create(Registries.CREATIVE_MODE_TAB, ResourceLocation.fromNamespaceAndPath(AlexsCaves.MODID, name));
    }
    ...
    public static final Supplier<CreativeModeTab> MAGNETIC_CAVES = DEF_REG.register("magnetic_caves", () -> CreativeModeTab.builder()
            ...
            .displayItems((enabledFeatures, output) -> {
                ...
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.GALENA_GAUNTLET);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.RESISTOR_SHIELD);
            })
            .build());

    public static final Supplier<CreativeModeTab> PRIMORDIAL_CAVES = DEF_REG.register("primordial_caves", () -> CreativeModeTab.builder()
            .title(Component.translatable("itemGroup.alexscaves.primordial_caves"))
            .icon(() -> new ItemStack(ACBlockRegistry.FLYTRAP.get()))
            //? if !fabric
            .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
            //? if !fabric
            .withTabsBefore(tabKey("magnetic_caves"))
            .displayItems((enabledFeatures, output) -> {
                ...
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.PRIMITIVE_CLUB);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.EXTINCTION_SPEAR);
            })
            .build());

    public static final Supplier<CreativeModeTab> TOXIC_CAVES = DEF_REG.register("toxic_caves", () -> CreativeModeTab.builder()
            .title(Component.translatable("itemGroup.alexscaves.toxic_caves"))
            .icon(() -> new ItemStack(ACBlockRegistry.WASTE_DRUM.get()))
            //? if !fabric
            .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
            //? if !fabric
            .withTabsBefore(tabKey("primordial_caves"))
            .displayItems((enabledFeatures, output) -> {
                ...
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.RAYGUN);
            })
            .build());

    public static final Supplier<CreativeModeTab> ABYSSAL_CHASM = DEF_REG.register("abyssal_chasm", () -> CreativeModeTab.builder()
            .title(Component.translatable("itemGroup.alexscaves.abyssal_chasm"))
            .icon(() -> new ItemStack(ACItemRegistry.SUBMARINE.get()))
            //? if !fabric
            .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
            //? if !fabric
            .withTabsBefore(tabKey("toxic_caves"))
            .displayItems((enabledFeatures, output) -> {
                ...
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.ORTHOLANCE);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.MAGIC_CONCH);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.SEA_STAFF);
            })
            .build());

    public static final Supplier<CreativeModeTab> FORLORN_HOLLOWS = DEF_REG.register("forlorn_hollows", () -> CreativeModeTab.builder()
            ...
            .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
            //? if !fabric
            .withTabsBefore(tabKey("abyssal_chasm"))
            .displayItems((enabledFeatures, output) -> {
                ...
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.TOTEM_OF_POSSESSION);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.DESOLATE_DAGGER);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.DREADBOW);
            })
            .build());

    public static final Supplier<CreativeModeTab> CANDY_CAVITY = DEF_REG.register("candy_cavity", () -> CreativeModeTab.builder()
            .title(Component.translatable("itemGroup.alexscaves.candy_cavity"))
            .icon(() -> new ItemStack(ACBlockRegistry.LARGE_PEPPERMINT.get()))
            //? if !fabric
            .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
            //? if !fabric
            .withTabsBefore(tabKey("forlorn_hollows"))
            .displayItems((enabledFeatures, output) -> {
                ...
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.SHOT_GUM);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.CANDY_CANE_HOOK);
                ACEnchantmentRegistry.addAllEnchantsToCreativeTab(enabledFeatures, output, ACEnchantmentRegistry.SUGAR_STAFF);
            })
            .build());
