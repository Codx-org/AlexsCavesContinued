...
public class BeholderBlockEntity extends BlockEntity implements ACUpdatePacketReceiver {
    ...
    public void startObserving(Level level, Player player) {
        BeholderEyeEntity beholderEyeEntity = ACCompat.createEntity(ACEntityRegistry.BEHOLDER_EYE.get(), level);
        ...
        ACCompat.displayClientMessage(player, Component.translatable("item.alexscaves.occult_gem.start_observing"), true);
    }

    public void load(CompoundTag tag) {
        super.load(tag);
        this.currentlyUsingEntityId = ACCompat.getInt(tag, "UsingEntityID");
    }
    ...
    public void onDataPacket(Connection net, ClientboundBlockEntityDataPacket packet) {
        if (packet != null && packet.getTag() != null) {
            this.currentlyUsingEntityId = ACCompat.getInt(packet.getTag(), "UsingEntityID");
        }
    }
