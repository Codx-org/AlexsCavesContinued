...
public class MineGuardianAnchorEntity extends Entity {
    ...
    *///?}
    ...
    private static final EntityDataAccessor<Optional<UUID>> GUARDIAN_UUID = SynchedEntityData.defineId(MineGuardianAnchorEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            if (guardian instanceof MineGuardianEntity mineGuardian) {
                ...
                double distanceGoal = (ACCompat.isInWaterOrBubble(mineGuardian) ? i + Math.sin(tickCount * 0.1F + i * 0.5F) * 0.25F : 5) + (hasTarget ? 5 : 0);
                double waterHeight = ACFluids.waterHeight(mineGuardian);
                double waterUp = Math.min(waterHeight, 1F) * 0.005F;
                if (ACCompat.isInWaterOrBubble(mineGuardian) && !hasTarget) {
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        if (ACCompat.hasUUID(compound, "GuardianUUID")) {
            this.setGuardianUUID(ACCompat.getUUID(compound, "GuardianUUID"));
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        if (this.getGuardianUUID() != null) {
            ACCompat.putUUID(compound, "GuardianUUID", this.getGuardianUUID());
        }
    }
