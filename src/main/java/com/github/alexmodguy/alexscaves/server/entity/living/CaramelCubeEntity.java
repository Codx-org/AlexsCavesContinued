...
public class CaramelCubeEntity extends Monster implements PossessedByLicowitch {
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(1, new NearestAttackableTargetGoal<>(this, Player.class, 10, true, false, ACCompat.targetSelector((y) -> {
    ...
    // public, not protected: 1.21 widened LivingEntity#jumpFromGround and an override may not
    // narrow. Widening early is legal on every older version too, so no gate is needed.
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setSlimeSize(ACCompat.getInt(compound, "SlimeSize"), false);
    ...
    private void spawnMeltedCaramel() {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            float f2 = Mth.sin(f) * (float) i * 0.5F * f1;
            float f3 = Mth.cos(f) * (float) i * 0.5F * f1;
            MeltedCaramelEntity meltedCaramel = ACCompat.createEntity(ACEntityRegistry.MELTED_CARAMEL.get(), level());
    ...
    *///?} else {
    ...
    // getDefaultDimensions; only the signature line differs, so the body is shared.
    //? if >=1.20.5 {
    ...
    public void remove(Entity.RemovalReason reason) {
        ...
        if (!this.level().isClientSide() && ourSize > 0 && this.isDeadOrDying()) {
            ...
            for (int l = 0; l < slimesSpawned; ++l) {
                ...
                CaramelCubeEntity slime = ACCompat.createEntity(ACEntityRegistry.CARAMEL_CUBE.get(), this.level());
                if (slime != null) {
                    ...
                    ACCompat.moveTo(slime, this.getX() + (double) f1, this.getY() + 0.5D, this.getZ() + (double) f2, this.random.nextFloat() * 360.0F, 0.0F);
