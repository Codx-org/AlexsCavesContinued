...
public class RelicheirusEntity extends DinosaurEntity implements IAnimatedEntity, CullingBoundsEntity {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return ACCompat.temptable(Monster.createMonsterAttributes().add(Attributes.ATTACK_DAMAGE, 12.0D).add(Attributes.MOVEMENT_SPEED, 0.2D).add(Attributes.MAX_HEALTH, 120.0D));
    }
    ...
    protected float getStandingEyeHeight(Pose pose, EntityDimensions dimensions) {
        return 0.99F * ACCompat.height(dimensions);
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return ACCompat.createEntity(ACEntityRegistry.RELICHEIRUS.get(), level);
    }
