...
//? if >=1.21.5 {
/*public class RainbounceBootsItem extends net.minecraft.world.item.Item implements CustomArmorPostRender, ACClientExtensionItem {
*///?} else {
public class RainbounceBootsItem extends ArmorItem implements CustomArmorPostRender, ACClientExtensionItem {
//?}
    // Takes the slot like the other five sets even though it is only ever BOOTS, so the
    // super() call reads identically everywhere and one rename rule can restate it for 1.21.5.
    public RainbounceBootsItem(ACArmorMaterial armorMaterial, ArmorItem.Type slot) {
    ...
    public static void onEntityLand(LivingEntity living, Vec3 vec3) {
        ...
        if (!living.isSuppressingBounce() && f > 0.2F && vec3.y < 0.0) {
            ...
            float xzInertia = living.hasEffect(ACCompat.effect(ACEffectRegistry.SUGAR_RUSH.get())) ? 1.2F : 1.9F;
