...
import com.github.alexmodguy.alexscaves.server.entity.util.ACMultipartOwner;

public class CorrodentEntity extends Monster implements ICustomCollisions, IAnimatedEntity, CullingBoundsEntity, ACMultipartOwner {
        ...
        if (isDigging() && surfacePosition != null) {
            if (level().isClientSide() && isMoving()) {
                ...
                if (surfaceState.isSolid()) {
                    ...
                    ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, surfaceState), head.x, head.y, head.z, random.nextFloat() - 0.5F, random.nextFloat() + 0.5F, random.nextFloat() - 0.5F);
                    for (int i = 0; i < 4 + random.nextInt(4); i++) {
                        ...
                        ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, surfaceState), offset.x + head.x, offset.y + head.y, offset.z + head.z, (random.nextFloat() - 0.5F) * 0.2F + offset.x, (random.nextFloat() - 0.5F) * 0.2F + offset.y, (random.nextFloat() - 0.5F) * 0.2F + offset.z);
                    }
                }
                if (onState.isSolid()) {
                    for (int i = 0; i < 2 + random.nextInt(2); i++) {
                        ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, onState), this.getRandomX(0.8F), this.getRandomY(), this.getRandomZ(0.8F), (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F, (random.nextFloat() - 0.5F) * 0.2F);
                    }
                }
            }
        }
        ...
        if (b == 77) {
            ...
            for (int i1 = 0; i1 < particleCount; i1++) {
                ...
                if (groundState.isSolid()) {
                    if (level().isClientSide()) {
                        ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, groundState), this.getX() + extraX, ground.getY() + extraY, this.getZ() + extraZ, motionX, motionY, motionZ);
                    }
                }
            }
        } else {
    ...
    static class DiggingNodeEvaluator extends FlyNodeEvaluator {
        ...
        // Both arms answer "what is this one cell worth to a corrodent", and both are called once
        // per cell of the mob's bounding box. Below 1.20.5 that hook is Forge's evaluateBlockPathType,
        // which post-processes the FlyNodeEvaluator's own answer; 1.20.5 deleted it because the
        // vanilla per-cell method became overridable at the same point. Either way the incoming
        // answer is discarded and the raw static type is recomputed, so the two are equivalent.
        //? if >=1.20.5 {
        /*public BlockPathTypes getPathType(net.minecraft.world.level.pathfinder.PathfindingContext context, int x, int y, int z) {
            BlockPos pos = new BlockPos(x, y, z);
            return digPathType(context.level(), pos,
                    net.minecraft.world.level.pathfinder.WalkNodeEvaluator.getPathTypeStatic(context, pos.mutable()));
        }
        *///?} else {
        protected BlockPathTypes evaluateBlockPathType(BlockGetter level, BlockPos pos, BlockPathTypes typeIn) {
            return digPathType(level, pos, getBlockPathTypeStatic(level, pos.mutable()));
        }
        //?}

        private BlockPathTypes digPathType(BlockGetter level, BlockPos pos, BlockPathTypes def) {
