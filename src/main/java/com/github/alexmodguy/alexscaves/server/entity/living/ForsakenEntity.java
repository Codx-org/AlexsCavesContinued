...
public class ForsakenEntity extends Monster implements IAnimatedEntity, ShakesScreen {
    ...
    public static final Predicate<LivingEntity> TARGETING = (mob) -> {
        return !mob.getType().builtInRegistryHolder().is(ACTagRegistry.FORSAKEN_IGNORES);
    };
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(3, new NearestAttackableTargetGoal<>(this, Mob.class, 120, false, true, ACCompat.targetSelector(TARGETING)));
    }
                ...
                if (this.getAnimationTick() == 12) {
                    ...
                    for (int i1 = 0; i1 < particleCount; i1++) {
                        ...
                        if (groundState.isSolid()) {
                            if (level().isClientSide()) {
                                ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, groundState), smashPos.x + extraX, ground.getY() + extraY, smashPos.z + extraZ, motionX, motionY, motionZ);
                            }
                        }
                    }
                }
    ...
    protected void customServerAiStep() {
        ...
        if(this.isInWall()){
            if (this.destroyBlocksTick > 0) {
                ...
                if (this.destroyBlocksTick == 0 && com.github.alexmodguy.alexscaves.server.misc.ACPlatform.canEntityGrief(this.level(), this)) {
                    ...
                    for(int j = -1; j <= 1; ++j) {
                        for(int k2 = -1; k2 <= 1; ++k2) {
                            for(int k = 0; k <= 3; ++k) {
                                ...
                                if (com.github.alexmodguy.alexscaves.server.misc.ACCompat.canEntityDestroy(blockstate, this.level(), blockpos, this) && !blockstate.is(ACTagRegistry.UNMOVEABLE) && net.minecraftforge.event.ForgeEventFactory.onEntityDestroyBlock(this, blockpos, blockstate)) {
    ...
    public float getSonicDamageAgainst(LivingEntity target) {
        return target.getType().builtInRegistryHolder().is(ACTagRegistry.WEAK_TO_FORSAKEN_SONIC_ATTACK) ? 45.0F : 4.0F;
    }
