...
public class SpelunkeryTableMenu extends AbstractContainerMenu {
    ...
    public SpelunkeryTableMenu(int id, Inventory inventory, final ContainerLevelAccess access) {
        ...
        this.addSlot(new Slot(this.resultContainer, 2, 142, 143) {
            ...
            public void onTake(Player player, ItemStack stack) {
                ...
                // 1.21.5 dropped the Level parameter — it is the player's own level, and the
                // system-crafted case that had no player is onCraftedBySystem now.
                //? if >=1.21.5
    ...
    private void setupResultSlot(ResourceKey<Biome> biomeResourceKey, Player player) {
        this.access.execute((p_39170_, p_39171_) -> {
            ...
            } else if (ACCompat.sameItemSameData(itemInFinalSlot, itemstack) && itemInFinalSlot.getCount() + itemstack.getCount() < itemInFinalSlot.getMaxStackSize()) {
    ...
    public static void setTutorialComplete(Player player, boolean done) {
        CompoundTag playerData = ACCompat.getPersistentData(player);
        CompoundTag data = ACCompat.getCompound(playerData, ACCompat.PERSISTED_NBT_TAG);
        if (data != null) {
            data.putBoolean(NEEDS_TUTORIAL_IDENTIFIER, done);
            playerData.put(ACCompat.PERSISTED_NBT_TAG, data);
        }
    }
    ...
    public static boolean hasCompletedTutorial(Player player) {
        CompoundTag playerData = ACCompat.getPersistentData(player);
        CompoundTag data = ACCompat.getCompound(playerData, ACCompat.PERSISTED_NBT_TAG);
        return data != null && ACCompat.getBoolean(data, NEEDS_TUTORIAL_IDENTIFIER);
    }
