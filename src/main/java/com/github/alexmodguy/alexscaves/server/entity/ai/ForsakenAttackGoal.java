...
public class ForsakenAttackGoal extends Goal {
    ...
    public void tick() {
        ...
        if (target != null && target.isAlive()) {
                    ...
                    if (this.entity.getAnimationTick() >= 10 && this.entity.getAnimationTick() <= 30) {
                        if (this.entity.getAnimationTick() % 5 == 0) {
                            ...
                            for (LivingEntity living : list) {
                                if (living != this.entity && !this.entity.isAlliedTo(living) && !living.isAlliedTo(entity) && living.distanceTo(entity) <= 14F && !living.getType().builtInRegistryHolder().is(ACTagRegistry.FORSAKEN_IGNORES)) {
            ...
            if (entity.getAnimation() == ForsakenEntity.ANIMATION_GROUND_SMASH && entity.getAnimationTick() >= 10 && entity.getAnimationTick() <= 15) {
                ...
                boolean flag = false;
                for (LivingEntity living : list) {
                    if (living != this.entity && !this.entity.isAlliedTo(living) && !living.isAlliedTo(entity) && living.distanceToSqr(smashPos) <= 16F && !living.getType().builtInRegistryHolder().is(ACTagRegistry.FORSAKEN_IGNORES)) {
    ...
    private boolean checkAndDealDamage(LivingEntity target, double multiplier, float extraRange) {
        if (entity.hasLineOfSight(target) && entity.distanceTo(target) < entity.getBbWidth() + target.getBbWidth() + extraRange) {
            boolean b = ACCompat.hurt(target, target.damageSources().mobAttack(entity), (float) (multiplier * entity.getAttribute(Attributes.ATTACK_DAMAGE).getValue()));
    ...
    private void knockBackAngle(LivingEntity target, double strength, float angle) {
        ...
        ACCompat.knockback(target, (double) strength, (double) Mth.sin(yRot * ((float) Math.PI / 180F)), (double) (-Mth.cos(yRot * ((float) Math.PI / 180F))));
    ...
    private BlockPos findJumpTarget(LivingEntity target, boolean far) {
        ...
        if (vec3 != null) {
            ...
            if (ACCompat.isSolidRender(entity.level().getBlockState(blockpos.below()), entity.level(), blockpos.below()) && entity.getPathfindingMalus(ACCompat.pathTypeStatic(this.entity, entity.level(), blockpos.mutable())) == 0.0F && entity.level().isUnobstructed(this.entity, Shapes.create(aabb))) {
                return blockpos;
            }
        }
        return null;
