...
public class TotemExplosion {
    ...
    public TotemExplosion(Level level, @Nullable Entity entity, double x, double y, double z, float radius, Explosion.BlockInteraction blockInteraction) {
        this.level = level;
        this.source = entity;
        this.radius = radius;
        this.x = x;
    ...
    public void finalizeExplosion(boolean particles) {
        this.level.playSound(null, this.x, this.y, this.z, ACCompat.rawSound(SoundEvents.GENERIC_EXPLODE), SoundSource.BLOCKS, 4.0F, (1.0F + (this.level.getRandom().nextFloat() - this.level.getRandom().nextFloat()) * 0.2F) * 0.7F);
        ...
        if (particles && !level.isClientSide() && level instanceof ServerLevel serverLevel) {
            for (int i = 0; i < 5 + radius * 10; i++) {
                ...
                serverLevel.sendParticles(ACParticleRegistry.TOTEM_EXPLOSION.get(), this.x + particleX, this.y + particleY, this.z + particleZ, 0, 0.0D, 0.0D, 0.0D, 1);
            }
        }
    }
