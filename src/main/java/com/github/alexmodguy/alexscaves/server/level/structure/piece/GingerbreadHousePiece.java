...
public class GingerbreadHousePiece extends UndergroundTemplateStructurePiece {
    ...
    public GingerbreadHousePiece(StructureTemplateManager manager, CompoundTag tag) {
        super(ACStructurePieceRegistry.GINGERBREAD_HOUSE.get(), tag, manager, (x) -> {
            return makeSettings(Rotation.valueOf(ACCompat.getString(tag, "Rotation")));
        });
    }
    ...
    protected void handleDataMarker(String string, BlockPos pos, ServerLevelAccessor accessor, RandomSource random, BoundingBox box) {
        if (string.equals("loot_chest")) {
            ACPlatform.setBlockEntityLootTable(accessor, random, pos.below(), ACLootTableRegistry.GINGERBREAD_TOWN_CHEST);
        }
        ...
        // 1.21.5 renamed LevelAccessor#blockUpdated to updateNeighborsAt; same default method, same
        // body. The old spelling is not kept as an alias, so this is a gate rather than a rename
        // rule — one site.
        //? if >=1.21.5
        /*accessor.updateNeighborsAt(pos, accessor.getBlockState(pos).getBlock());*/
        //? if <1.21.5
