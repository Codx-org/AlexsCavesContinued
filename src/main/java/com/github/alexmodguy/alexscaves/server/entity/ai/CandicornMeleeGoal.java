        ...
        if (target != null && target.isAlive()) {
            ...
            if (candicorn.getAnimation() == CandicornEntity.ANIMATION_STAB) {
                ...
                if(candicorn.getAnimationTick() > 8 && candicorn.getAnimationTick() <= 12){
                    if(ACCompat.hurt(target, target.damageSources().mobAttack(candicorn), (float) candicorn.getAttribute(Attributes.ATTACK_DAMAGE).getValue())){
                        ACCompat.knockback(target, 0.6F, candicorn.getX() - target.getX(), candicorn.getZ() - target.getZ());
                    }
                }
            } else {
