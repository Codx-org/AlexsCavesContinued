...
public class UnrefinedWasteBlock extends FallingBlockWithColor {
    ...
    public void entityInside(BlockState state, Level level, BlockPos blockPos, Entity entity) {
        if(entity instanceof LivingEntity living && !entity.getType().builtInRegistryHolder().is(ACTagRegistry.RESISTS_RADIATION)){
            ...
            living.addEffect(new MobEffectInstance(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()), 4000));
        }
    }
    ...
    public void animateTick(BlockState blockState, Level level, BlockPos blockPos, RandomSource randomSource) {
        if (randomSource.nextInt(2) == 0) {
            ...
            if (!blockState.canOcclude() || !blockstate.isFaceSturdy(level, blockpos, direction.getOpposite())) {
                ...
                level.addParticle(randomSource.nextBoolean() ? ACParticleRegistry.GAMMAROACH.get() : ACParticleRegistry.HAZMAT_BREATHE.get(), (double) blockPos.getX() + d0, (double) blockPos.getY() + d1, (double) blockPos.getZ() + d2, 0.0D, 0.1D + level.getRandom().nextFloat() * 0.1F, 0.0D);
            }
        }
    }
}
