...
public class BoundroidWinchEntity extends Monster {
    ...
    private static final EntityDataAccessor<Optional<UUID>> HEAD_UUID = SynchedEntityData.defineId(BoundroidWinchEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public boolean hurt(DamageSource source, float damage) {
        ...
        // Neither of these is the in-override self-call the !mc2102-invulnerable-* rules describe —
        // one asks the winch about itself from outside its own isInvulnerableTo, the other asks the
        // boundroid it is latched to — so both go through ACCompat, which is version-neutral.
        if (ACCompat.isInvulnerableTo(this, source)) {
            return false;
        }
        if (body != null && !ACCompat.isInvulnerableTo(body, source)) {
            boolean flag = ACCompat.hurt(body, source, damage);
            if (flag) {
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        if (this.getHeadUUID() != null) {
            ACCompat.putUUID(compound, "BodyUUID", this.getHeadUUID());
        }
    }
