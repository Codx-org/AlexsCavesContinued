...
public class RaycatEntity extends TamableAnimal implements IComandableMob {
    ...
    public void tick() {
        ...
        if (this.hasEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get())) && this.tickCount % 10 == 0) {
            this.heal(1);
        }
        if (absorbCooldown > 0) {
            absorbCooldown--;
            ...
            if (absorbTarget == null) {
                if (!level().isClientSide()) {
                    ...
                    if (owner != null && owner.distanceTo(this) < 20 && owner.hasEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()))) {
                        ...
                        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(9D))) {
                            if (!(entity instanceof RaycatEntity) && entity.hasEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get())) && (closestIrradiated == null || closestIrradiated.distanceTo(this) > entity.distanceTo(this))) {
                ...
                if (getAbsorbAmount() <= 0) {
                    ...
                    if (getAbsorbAmount() <= 0) {
                        int currentRad = this.hasEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get())) ? this.getEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get())).getAmplifier() + 1 : 0;
                        this.heal(10);
                        this.addEffect(new MobEffectInstance(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()), 200, currentRad));
                        this.lookAt(absorbTarget, 30, 30);
                        if (absorbTarget instanceof LivingEntity living) {
                            MobEffectInstance effectInstance = living.getEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()));
                            if (effectInstance != null) {
                                int timeLeft = effectInstance.getDuration();
                                int level = effectInstance.getAmplifier();
                                living.removeEffect(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()));
                                if (level > 0) {
                                    living.addEffect(new MobEffectInstance(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()), timeLeft, level - 1));
                                }
                            }
                        }
                        setAbsorbTargetId(-1);
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setLayTime(ACCompat.getInt(compound, "LayTime"));
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob mob) {
        return ACCompat.createEntity(ACEntityRegistry.RAYCAT.get(), serverLevel);
    }
