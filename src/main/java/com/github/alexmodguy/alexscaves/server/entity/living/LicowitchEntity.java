...
public class LicowitchEntity extends Monster implements IAnimatedEntity {
    ...
    private static final EntityDataAccessor<Optional<UUID>> POSSESSED_UUID_0 = SynchedEntityData.defineId(LicowitchEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    // ⚠️ Built on first use, and it has to stay that way. Upstream initialised this in the class
    // initialiser, which is only safe while an ItemStack can be constructed before the item
    // registry's component maps are bound. From 26.2 ItemStack's constructor reads
    // Holder.Reference#components eagerly and throws "Components not bound yet" — and this class is
    // loaded from ACEntityRegistry#initializeAttributes, which names createAttributes while
    // EntityAttributeCreationEvent fires, i.e. well before the registries freeze. The resulting
    // stack is identical on every node, so the laziness is not gated.
    //
    // Only the licowitch's own goal reads it, on the server thread, so no synchronisation is
    // needed; a racing second build would produce an equal stack anyway.
    private static ItemStack hungerPotion;

    public static ItemStack hungerPotion() {
        if (hungerPotion == null) {
            hungerPotion = ACEffectRegistry.createSplashPotion(ACEffectRegistry.STRONG_HUNGER_POTION.get());
        }
        return hungerPotion;
    }
    ...
    public LicowitchEntity(EntityType<? extends Monster> entityType, Level level) {
        ...
        // Upstream overrode getEquipmentDropChance to force the hands to zero and leave the armour
        // at its default. 1.21.5 replaced that method with the immutable DropChances record, so the
        // same thing is said once here instead — identical on every version, and setDropChance is
        // the one part of that API that survived unchanged.
    ...
    public static boolean isWithinTowerSpawnBounds(ServerLevelAccessor level, BlockPos pos) {
        // 1.21.2 rotated Registry#get to mean "the Holder" and named the nullable value getValue.
        // The rename rules cover that swap for the registries this mod reads by id in bulk; this is
        // the one lookup off a datapack registry, so it says it here. Same value, still nullable.
        //? if >=1.21.2 {
        /*Structure structure = level.registryAccess().lookupOrThrow(Registries.STRUCTURE).getValue(ResourceLocation.fromNamespaceAndPath(AlexsCaves.MODID, "licowitch_tower"));
        *///?} else {
        Structure structure = level.registryAccess().registryOrThrow(Registries.STRUCTURE).get(ResourceLocation.fromNamespaceAndPath(AlexsCaves.MODID, "licowitch_tower"));
        //?}
    ...
    public void tick() {
        ...
        if (this.getAnimation() == ANIMATION_EAT && this.getAnimationTick() < 90 && this.getAnimationTick() % 4 == 0) {
            // See ACCompat#emitItemUseEffects: 1.21.2 moved the eating burst onto the item's
            // consumable component, and the method this used to call is protected, so it gates here.
            //? if >=1.21.2 {
            /*ACCompat.emitItemUseEffects(this, this.getItemInHand(InteractionHand.MAIN_HAND), 2);
            *///?} else {
    ...
    public void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        for (UUID uuid : list) {
            if (uuid != null) {
                // NbtUtils.createUUID/loadUUID are gone in 1.21.5; this is their body verbatim,
                // so the stored form is identical and no gate is needed.
                listtag.add(new net.minecraft.nbt.IntArrayTag(net.minecraft.core.UUIDUtil.uuidToIntArray(uuid)));
            }
        }
    ...
    public void readAdditionalSaveData(CompoundTag compoundTag) {
        ...
        ListTag listtag = ACCompat.getList(compoundTag, "Possessed", 11);
        for (int i = 0; i < listtag.size(); ++i) {
            this.addPossessedUUID(net.minecraft.core.UUIDUtil.uuidFromIntArray(((net.minecraft.nbt.IntArrayTag) listtag.get(i)).getAsIntArray()));
        }
        if (compoundTag.contains("CrucibleX") && compoundTag.contains("CrucibleY") && compoundTag.contains("CrucibleZ")) {
            this.setLastCruciblePos(new BlockPos(ACCompat.getInt(compoundTag, "CrucibleX"), ACCompat.getInt(compoundTag, "CrucibleY"), ACCompat.getInt(compoundTag, "CrucibleZ")));
        }
        teleportCooldown = ACCompat.getInt(compoundTag, "TeleportCooldown");
    }
    ...
    public Mob createRandomPossessedMob() {
        ...
        if (chance < 0.1F) {
            return ACCompat.createEntity(ACEntityRegistry.CANDICORN.get(), level());
        } else if (chance < 0.2F) {
            return ACCompat.createEntity(ACEntityRegistry.GUMMY_BEAR.get(), level());
        } else if (chance < 0.5F) {
            return ACCompat.createEntity(ACEntityRegistry.GUMBEEPER.get(), level());
        } else if (chance < 0.65F) {
            return ACCompat.createEntity(ACEntityRegistry.CARAMEL_CUBE.get(), level());
        } else if (chance < 0.75F) {
            return ACCompat.createEntity(ACEntityRegistry.GINGERBREAD_MAN.get(), level());
        } else {
            return ACCompat.createEntity(ACEntityRegistry.CANIAC.get(), level());
        }
    }
    ...
    // 1.21.4 dropped the score amount from the signature — the value is read off the killed entity now.
    //? if >=1.21.4 {
    /*@Override
    ...
    *///?} else {
    ...
    //?}
        this.level().playSound((Player) null, this.blockPosition(), ACSoundRegistry.LICOWITCH_CELEBRATE.get(), SoundSource.HOSTILE, 0.3F, 0.9F + this.level().getRandom().nextFloat() * 0.2F);
    }
