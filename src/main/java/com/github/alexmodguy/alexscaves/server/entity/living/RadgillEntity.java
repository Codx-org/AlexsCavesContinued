...
public class RadgillEntity extends WaterAnimal implements Bucketable {
    ...
    protected PathNavigation createNavigation(Level level) {
        return new WaterBoundPathNavigation(this, level) {
            ...
            // Upstream overrode PathNavigation#isInLiquid, which 1.20.2 deleted — WaterBoundPathNavigation
            // now asks the mob directly. canUpdatePath is the caller and exists unchanged on every
            // version, and on 1.20.1 its whole body was `return this.isInLiquid()`, so overriding it
            // one level up means the same thing everywhere.
            protected boolean canUpdatePath() {
                return RadgillEntity.this.isInBreathableFluid();
            }
        };
    }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setFromBucket(ACCompat.getBoolean(compound, "FromBucket"));
    }
    ...
    public static boolean checkRadgillSpawnRules(EntityType<? extends LivingEntity> type, ServerLevelAccessor level, MobSpawnType spawnType, BlockPos pos, RandomSource randomSource) {
        return spawnType == MobSpawnType.SPAWNER || !level.getFluidState(pos).isEmpty() && level.getFluidState(pos).is(ACTagRegistry.ACID);
    }
    ...
    public void tick() {
        ...
        boolean grounded = this.onGround() && !isInBreathableFluid();
        ...
        if (!isInBreathableFluid() && this.isAlive()) {
    ...
    // Not an override: 1.20.2 added a public Entity#isInLiquid (water or lava), which this
    // predates and does not mean.
    private boolean isInBreathableFluid() {
    ...
    private boolean isInAcid() {
        return ACFluids.acidHeight(this) > 0;
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!type.consumesAction()) {
            if (itemstack.getItem() == ACItemRegistry.ACID_BUCKET.get() && this.isAlive()) {
                ...
                this.discard();
                return ACCompat.sidedSuccess(this.level().isClientSide());
            }
        }
        return type;
    }
    ...
    public void saveToBucketTag(@Nonnull ItemStack bucket) {
        if (this.hasCustomName()) {
            ACCompat.setHoverName(bucket, this.getCustomName());
        }
        CompoundTag platTag = new CompoundTag();
        this.addAdditionalSaveData(ACCompat.asOutput(platTag, this.level().registryAccess()));
        CompoundTag compound = ACCompat.getOrCreateTag(bucket);
        compound.put("FishBucketTag", platTag);
        ACCompat.setTag(bucket, compound);
    }

    @Override
    public void loadFromBucketTag(@Nonnull CompoundTag compound) {
        if (compound.contains("FishBucketTag")) {
            this.readAdditionalSaveData(ACCompat.asInput(ACCompat.getCompound(compound, "FishBucketTag"), this.level().registryAccess()));
        }
    ...
    public ItemStack getBucketItemStack() {
        ...
        if (this.hasCustomName()) {
            ACCompat.setHoverName(stack, this.getCustomName());
        }
        return stack;
    }
    ...
    private class WanderGoal extends Goal {
        ...
        private boolean isLiquidAt(BlockPos pos) {
            ...
            return state.is(FluidTags.WATER) || state.is(ACTagRegistry.ACID);
        }
        ...
        public boolean canUse() {
            if (!RadgillEntity.this.isInBreathableFluid()) {
                return false;
        ...
        public boolean canContinueToUse() {
            return (RadgillEntity.this.isInBreathableFluid() && !hasJumped || isJump) && RadgillEntity.this.distanceToSqr(Vec3.atCenterOf(target)) < 3 && timeout < 200;
        }
    ...
    class AcidMoveControl extends MoveControl {
        ...
        public void tick() {
            if (this.operation == MoveControl.Operation.MOVE_TO && RadgillEntity.this.isInBreathableFluid()) {
