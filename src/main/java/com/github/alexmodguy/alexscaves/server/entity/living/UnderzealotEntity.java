    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            if (this.isBuried() && buriedProgress >= 20.0F) {
                if (reemergeTime-- < 0) {
                    ACCompat.moveTo(this, remergePos.getX() + 0.5F, remergePos.getY(), remergePos.getZ() + 0.5F);
            ...
            if (stateOn.isSolid()) {
                for (int i = 0; i < 3; i++) {
                    ACCompat.addParticleForced(level(), new BlockParticleOption(ParticleTypes.BLOCK, stateOn), this.getRandomX(0.8F), this.getY(), this.getRandomZ(0.8F), random.nextFloat() - 0.5F, random.nextFloat() + 0.5F, random.nextFloat() - 0.5F);
                }
            }
            if (isBuried()) {
    ...
    *///?} else {
    ...
    private void spawnReinforcements(ServerLevelAccessor worldIn) {
        for (int i = 0; i < 3 + random.nextInt(2); i++) {
            UnderzealotEntity friend = ACCompat.createEntity(ACEntityRegistry.UNDERZEALOT.get(), worldIn.getLevel());
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.sacrificeCooldown = ACCompat.getInt(compound, "SacrificeCooldown");
        this.setBuried(ACCompat.getBoolean(compound, "Buried"));
        if (compound.contains("RX") && compound.contains("RY") && compound.contains("RZ")) {
            this.remergePos = new BlockPos(ACCompat.getInt(compound, "RX"), ACCompat.getInt(compound, "RY"), ACCompat.getInt(compound, "RZ"));
        }
        if (compound.contains("SX") && compound.contains("SY") && compound.contains("SZ")) {
            this.setLastSacrificePos(new BlockPos(ACCompat.getInt(compound, "SX"), ACCompat.getInt(compound, "SY"), ACCompat.getInt(compound, "SZ")));
        }
    }
