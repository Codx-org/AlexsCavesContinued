...
public class SauropodPartEntity extends PartEntity<SauropodBaseEntity> implements CullingBoundsEntity {
    ...
    public InteractionResult interact(Player player, InteractionHand hand) {
        ...
        if (parent == null) {
            ...
            this.playSound(ACCompat.rawSound(SoundEvents.ITEM_BREAK));
    ...
    // 1.21.2 replaced Entity#isInvulnerableTo with a protected final isInvulnerableToBase, so from
    // that version this is the mod's own method rather than an override and its super call has
    // nowhere to go. Hand-gated, because the !mc2102-invulnerable-* rules describe the LivingEntity
    // half of the split only; the >=1.21.2 arm spells DamageSource out in full precisely so those
    // rules cannot match it. The parent is a LivingEntity and goes through ACCompat on all versions.
    //? if >=1.21.2 {
    /*public boolean isInvulnerableTo(net.minecraft.world.damagesource.DamageSource damageSource) {
        SauropodBaseEntity parent = this.getParent();
        return this.isInvulnerableToBase(damageSource) || parent != null && ACCompat.isInvulnerableTo(parent, damageSource) || damageSource.getEntity() != null && this.getParent().isPassengerOfSameVehicle(damageSource.getEntity());
    }
    *///?} else {
    public boolean isInvulnerableTo(DamageSource damageSource) {
        SauropodBaseEntity parent = this.getParent();
        return super.isInvulnerableTo(damageSource) || parent != null && ACCompat.isInvulnerableTo(parent, damageSource) || damageSource.getEntity() != null && this.getParent().isPassengerOfSameVehicle(damageSource.getEntity());
    }
    //?}
    ...
    public boolean hurt(DamageSource source, float amount) {
        ...
        // See ACMultipartEntity#hurt for why 1.21.2 drops the client round-trip.
        //? if >=1.21.2 {
        /*if (!this.isInvulnerableTo(source) && parent != null) {
            Entity player = source.getEntity();
            if (player != null && !parent.isAlliedTo(player)) {
