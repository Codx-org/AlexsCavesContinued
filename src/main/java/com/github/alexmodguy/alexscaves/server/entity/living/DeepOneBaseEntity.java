...
public abstract class DeepOneBaseEntity extends PathfinderMob implements IAnimatedEntity {
    ...
    public void tick() {
        ...
        boolean water = ACCompat.isInWaterOrBubble(this);
            ...
            if (altarPos != null) {
                ...
                if (this.getAnimationTick() > getTradingAnimation().getDuration() - 10) {
                    if (level().getBlockEntity(altarPos) instanceof AbyssalAltarBlockEntity altar && !spawnedLootItem) {
                        ...
                        for (Player player : ACCompat.getNearbyPlayers(level(), TargetingConditions.forNonCombat().range(advancementRange), this, this.getBoundingBox().inflate(advancementRange))) {
    ...
    private List<ItemStack> generateBarterLoot() {
        LootTable loottable = ACCompat.lootTable(this.level().getServer(), getBarterLootTable());
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        if (!swappedItem.isEmpty()) {
            compound.put("SwappedItem", ACCompat.saveStack(this.level().registryAccess(), swappedItem));
        }
        compound.putBoolean("ConchSummoned", this.isSummoned());
        if (summonerUUID != null) {
            ACCompat.putUUID(compound, "ConchUUID", summonerUUID);
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (compound.contains("AltarX") && compound.contains("AltarY") && compound.contains("AltarZ")) {
            this.setLastAltarPos(new BlockPos(ACCompat.getInt(compound, "AltarX"), ACCompat.getInt(compound, "AltarY"), ACCompat.getInt(compound, "AltarZ")));
        }
        if (compound.contains("SwappedWeapon")) {
            swappedItem = ACCompat.loadStack(this.level().registryAccess(), ACCompat.getCompound(compound, "SwappedWeapon"));
        }
        this.setSummoned(ACCompat.getBoolean(compound, "ConchSummoned"));
        if (compound.contains("ConchUUID")) {
            this.summonerUUID = ACCompat.getUUID(compound, "ConchUUID");
            this.setSummonTime(ACCompat.getInt(compound, "ConchTime"));
        }
    }
    ...
    public void travel(Vec3 travelVector) {
        if (this.isEffectiveAi() && ACCompat.isInWaterOrBubble(this)) {
    ...
    *///?} else {
    ...
    public void addReputation(UUID playerUUID, int amount) {
        ...
        if (DeepOneReaction.fromReputation(prev) != newReaction) {
            ...
            if (player != null) {
                ...
                ACCompat.displayClientMessage(player, Component.translatable("entity.alexscaves.deep_one.reaction_" + newReaction.toString().toLowerCase(Locale.ROOT)), true);
            }
        }
    }
    ...
    protected void checkAndDealMeleeDamage(LivingEntity target, float multiplier, float knockback) {
        if (this.hasLineOfSight(target) && this.distanceTo(target) < this.getBbWidth() + target.getBbWidth() + 5.0D) {
            ...
            ACCompat.knockback(target, knockback * multiplier, this.getX() - target.getX(), this.getZ() - target.getZ());
    ...
    // 1.21.2 made Entity#isAlliedTo(Entity) final: it now asks both entities the new protected
    // considersEntityAsAlly hook and ORs the answers, so an override states only its own side of
    // the alliance and vanilla makes it symmetric. This body always meant "do *I* consider it an
    // ally", so it moves across as-is — only the header and the super call change name.
    //? if >=1.21.2
    ...
    public boolean isAlliedTo(Entity entityIn) {
        if (entityIn instanceof DeepOneBaseEntity) {
            ...
            return getReactionTo((Player) entityIn) == DeepOneReaction.HELPFUL || superAlliedTo(entityIn);
        } else {
            return superAlliedTo(entityIn);
        }
    }

    /** {@code super}'s own answer, under whichever name it goes by. */
    private boolean superAlliedTo(Entity entityIn) {
        //? if >=1.21.2 {
    ...
    private class PathNavigator extends SemiAquaticPathNavigator {
        ...
        protected double getGroundY(Vec3 vec3) {
            if (isDeepOneSwimming() || !ACCompat.isInWaterOrBubble(DeepOneBaseEntity.this)) {
