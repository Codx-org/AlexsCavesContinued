...
// 1.21.5 deleted net.minecraft.world.entity.Saddleable outright — a saddle is an equipment-slot
// item now. A candicorn never stored one anyway (it tracks a synched boolean and equips from
// mobInteract directly), so the interface simply comes off; the three members it declared stay.
//? if >=1.21.5 {
/*public class CandicornEntity extends TamableAnimal implements KeybindUsingMount, IAnimatedEntity, PlayerRideableJumping, RidingMeterMount, PossessedByLicowitch {
*///?} else {
...
//?}
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return ACCompat.temptable(Monster.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, 0.25D).add(Attributes.MAX_HEALTH, 30.0D).add(Attributes.ATTACK_DAMAGE, 6.0D).add(Attributes.FOLLOW_RANGE, 64.0D));
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel serverLevel, AgeableMob ageableMob) {
        CandicornEntity candicornEntity = ACCompat.createEntity(ACEntityRegistry.CANDICORN.get(), serverLevel);
    ...
    // 1.21 passes the saddle stack in so an item's components can travel onto the mob. A candicorn
    // stores nothing but a boolean, so the newer arm accepts the stack and ignores it. From 1.21.5
    // there is no interface left to override, but the method stays as the one place that plays the
    // sound and flips the flag together.
    //? if >=1.21.5 {
    /*public void equipSaddle(net.minecraft.world.item.ItemStack acSaddle, @Nullable SoundSource soundSource) {
        acEquipSaddle(soundSource);
    }
    *///?} elif >=1.21 {
    /*@Override
    public void equipSaddle(net.minecraft.world.item.ItemStack acSaddle, @Nullable SoundSource soundSource) {
    ...
    *///?} else {
    ...
    private void acEquipSaddle(@Nullable SoundSource soundSource) {
        this.setSaddled(true);
        if (soundSource != null) {
            this.level().playSound((Player)null, this, ACCompat.rawSound(SoundEvents.PIG_SADDLE), soundSource, 0.5F, 1.0F);
        }
    }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setCommand(ACCompat.getInt(compound, "Command"));
        this.setSaddled(ACCompat.getBoolean(compound, "Saddled"));
        this.setVariant(ACCompat.getInt(compound, "Variant"));
    }
        ...
        if(this.isCharging()){
            ...
            for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(3.5D))) {
                ...
                if (!(this.isTame() && isAlliedTo(entity)) && !entity.isPassengerOfSameVehicle(this) && !(entity instanceof CandicornEntity) && entity != this && dist < 7.0F) {
                    ...
                    ACCompat.knockback(entity, 2.0F, chargeFocalPoint.x - entity.getX(), chargeFocalPoint.z - entity.getZ());
                }
            }
            if(!this.isVehicle()){
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!interactionresult.consumesAction() && !type.consumesAction()) {
            if (itemstack.is(Items.SHEARS) && this.isSaddled() && isTame() && isOwnedBy(player)) {
                ...
                ACCompat.spawnAtLocation(this, Items.SADDLE);
                return InteractionResult.SUCCESS;
                ...
                if (player.isShiftKeyDown()) {
                    ...
                    ACCompat.displayClientMessage(player, Component.translatable("entity.alexscaves.all.command_" + this.getCommand(), this.getName()), true);
    ...
    public boolean causeFallDamage(float f1, float f2, DamageSource damageSource) {
        // The casts are no-ops below 1.21.5; from 1.21.5 the fall distance arrives as a double
        // (see the !mc2105-causefalldamage rule) while the loader hook and the private helper
        // below keep taking a float, which is all the precision the damage formula ever used.
        float[] ret = ACPlatform.onLivingFall(this, (float) f1, f2);
        if (ret == null) return false;
        f1 = ret[0];
        f2 = ret[1];

        boolean flag = causeInternalFallDamage((float) f1, f2, damageSource);
    ...
    private boolean causeInternalFallDamage(float f1, float f2, DamageSource damageSource) {
        float[] ret = ACPlatform.onLivingFall(this, f1, f2);
        if (ret == null) return false;
        f1 = ret[0];
        f2 = ret[1];
        ...
        if (i > 0) {
            ...
            return this.getType().builtInRegistryHolder().is(EntityTypeTags.FALL_DAMAGE_IMMUNE);
        }
    }
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger) && passenger instanceof LivingEntity living && !this.touchingUnloadedChunk()) {
            ...
            moveFunction.accept(passenger, this.getX() + seatOffset.x, this.getY() + seatOffset.y + ACPlatform.passengersRidingOffset(this, passenger), this.getZ() + seatOffset.z);
        } else {
    ...
    *///?} else {
    ...
    protected void playStepSound(BlockPos blockPos, BlockState blockState) {
        if (!blockState.liquid()) {
            ...
            SoundType soundtype = ACCompat.soundType(blockState, level(), blockPos, this);
            if (blockstate.is(Blocks.SNOW)) {
                soundtype = ACCompat.soundType(blockstate, level(), blockPos, this);
            }
            if (this.isVehicle()) {
