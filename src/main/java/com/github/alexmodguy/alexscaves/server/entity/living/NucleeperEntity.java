...
// Explicit, not covered by the wildcard above: 1.21.2 deletes this type and the
// !mc2102-render-import-powerable rule can only rewrite a single-type import.
...
public class NucleeperEntity extends Monster implements ActivatesSirens, PowerableMob {
    ...
    public void readAdditionalSaveData(CompoundTag compoundTag) {
        ...
        this.setCharged(ACCompat.getBoolean(compoundTag, "Charged"));
        this.setCloseTime(ACCompat.getInt(compoundTag, "CloseTime"));
    }
    ...
    private void explode() {
        ...
        if(!ACCompat.gameRule(level(), GameRules.RULE_MOBGRIEFING, true)){
    ...
    protected void dropCustomDeathLoot(DamageSource damageSource, int experience, boolean idk) {
        ...
        if (damageSource.getEntity() instanceof TremorzillaEntity && damageSource.is(ACDamageTypes.TREMORZILLA_BEAM)) {
            ACCompat.spawnAtLocation(this, ACItemRegistry.MUSIC_DISC_FUSION_FRAGMENT.get());
        }
    }
