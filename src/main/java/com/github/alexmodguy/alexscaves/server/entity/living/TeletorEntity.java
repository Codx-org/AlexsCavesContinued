...
public class TeletorEntity extends Monster {
    ...
    private static final EntityDataAccessor<Optional<UUID>> WEAPON_UUID = SynchedEntityData.defineId(TeletorEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        if (ACCompat.hasUUID(compound, "WeaponUUID")) {
            this.setWeaponUUID(ACCompat.getUUID(compound, "WeaponUUID"));
        }
    }
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        ...
        if (this.getWeaponUUID() != null) {
            ACCompat.putUUID(compound, "WeaponUUID", this.getWeaponUUID());
        }
    }
    ...
    *///?} else {
    ...
    //?}
        MagneticWeaponEntity magneticWeapon = ACCompat.createEntity(ACEntityRegistry.MAGNETIC_WEAPON.get(), this.level());
        ItemStack stack = createItemStack(level().getRandom());
        float f = difficultyIn.getSpecialMultiplier();
        if (level().getRandom().nextFloat() < 0.25F * (f + 0.5F)) {
            stack = ACCompat.enchantRandomly(level().getRandom(), stack, (int) (5.0F + f * (float) level().getRandom().nextInt(18)), level());
        }
    ...
    protected void dropEquipment() {
        ...
        if (weapon instanceof MagneticWeaponEntity magneticWeapon) {
            ...
            float f = ACCompat.equipmentDropChance(this, EquipmentSlot.MAINHAND);
            if (!itemstack.isEmpty() && !ACCompat.hasVanishingCurse(itemstack) && this.random.nextFloat() < f) {
                if (itemstack.isDamageableItem()) {
                    itemstack.setDamageValue(itemstack.getMaxDamage() - this.random.nextInt(1 + this.random.nextInt(Math.max(itemstack.getMaxDamage() - 3, 1))));
                }
                ACCompat.spawnAtLocation(this, itemstack);
            }
