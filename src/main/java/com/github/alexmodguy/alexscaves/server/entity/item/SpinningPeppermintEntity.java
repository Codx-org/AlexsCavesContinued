...
public class SpinningPeppermintEntity extends Entity {
    ...
    *///?}
    ...
    protected void readAdditionalSaveData(CompoundTag compoundTag) {
        this.setStraight(ACCompat.getBoolean(compoundTag, "Straight"));
        if (compoundTag.contains("DespawnsIn")) {
            this.despawnsIn = ACCompat.getInt(compoundTag, "DespawnsIn");
        }
        this.setLifespan(ACCompat.getInt(compoundTag, "Lifespan"));
        if (ACCompat.hasUUID(compoundTag, "Owner")) {
            this.ownerUUID = ACCompat.getUUID(compoundTag, "Owner");
        }
        if (compoundTag.contains("AroundX") && compoundTag.contains("AroundY") && compoundTag.contains("AroundZ")) {
            this.setSpinAroundPosition(new Vec3(ACCompat.getDouble(compoundTag, "AroundX"), ACCompat.getDouble(compoundTag, "AroundZ"), ACCompat.getDouble(compoundTag, "AroundZ")));
        }
        this.setSpinSpeed(ACCompat.getFloat(compoundTag, "SpinSpeed"));
        this.setSpinRadius(ACCompat.getFloat(compoundTag, "SpinRadius"));
        this.setStartAngle(ACCompat.getFloat(compoundTag, "StartAngle"));
        this.spinAngle = ACCompat.getFloat(compoundTag, "SpinAngle");
    }
    ...
    protected void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (this.ownerUUID != null) {
            ACCompat.putUUID(compoundTag, "Owner", this.ownerUUID);
        }
    ...
    private void hurtEntities() {
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
            if (!isAlliedTo(entity) && (owner != null && !entity.is(owner) && (!(owner instanceof LicowitchEntity) && !entity.isAlliedTo(owner) || owner instanceof LicowitchEntity licowitch && !licowitch.isFriendlyFire(entity)))) {
                if (ACCompat.hurt(entity, source, 3.0F)) {
                    flag = true;
                    ACCompat.knockback(entity, 0.3F, this.getX() - entity.getX(), this.getZ() - entity.getZ());
                }
            }
        }
