...
public class BrainiacEntity extends Monster implements IAnimatedEntity {
    ...
    public void tick() {
        ...
        if (!level().isClientSide() && this.hasBarrel()) {
            ...
            if (this.getAnimation() == ANIMATION_THROW_BARREL && this.getAnimationTick() >= 15) {
                ...
                if (attackTarget != null && attackTarget.isAlive()) {
                    ...
                    ThrownWasteDrumEntity wasteDrumEntity = ACCompat.createEntity(ACEntityRegistry.THROWN_WASTE_DRUM.get(), level());
        ...
        if (tongueTarget instanceof LivingEntity living) {
            if (this.shootTongueAmount >= 5F) {
                ...
                ACCompat.knockback(living, 0.3D, living.getX() - this.getX(), tongueTarget.getZ() - this.getZ());
                this.setLickTicks(0);
            }
        }
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setHasBarrel(ACCompat.getBoolean(compound, "HasBarrel"));
    }
    ...
    protected void dropEquipment() {
        ...
        if (this.hasBarrel() && shouldDropBlocks()) {
            ACCompat.spawnAtLocation(this, new ItemStack(ACBlockRegistry.WASTE_DRUM.get()));
        }
    }
    ...
    public void postAttackEffect(LivingEntity entity) {
        if (entity != null && entity.isAlive()) {
            entity.addEffect(new MobEffectInstance(ACCompat.effect(ACEffectRegistry.IRRADIATED.get()), 400));
        }
    }
    ...
    private class MeleeGoal extends Goal {
        ...
        private void checkAndDealDamage(LivingEntity target, float damageMultiplier) {
            if (BrainiacEntity.this.hasLineOfSight(target) && BrainiacEntity.this.distanceTo(target) < BrainiacEntity.this.getBbWidth() + target.getBbWidth() + 2.0D) {
                ...
                ACCompat.knockback(target, 0.3D, BrainiacEntity.this.getX() - target.getX(), BrainiacEntity.this.getZ() - target.getZ());
            }
        }
