...
public class WaveEntity extends Entity {
    ...
    *///?}
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(WAVE_SCALE, 1F);
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        if (ACCompat.hasUUID(tag, "Owner")) {
            this.ownerUUID = ACCompat.getUUID(tag, "Owner");
        }
        if (tag.contains("Lifespan")) {
            this.setLifespan(ACCompat.getInt(tag, "Lifespan"));
        }
    }
    ...
    public void tick() {
        ...
        if (!this.isNoGravity() && !ACCompat.isInWaterOrBubble(this)) {
    ...
    private void attackEntities(float scale) {
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
            if (!isAlliedTo(entity) && !(entity instanceof DeepOneBaseEntity) && (owner == null || !owner.equals(entity) && !owner.isAlliedTo(entity))) {
                ...
                this.setSlamming(true);
                ACCompat.knockback(entity, 0.1D + 0.5D * scale, (double) Mth.sin(this.getYRot() * ((float) Math.PI / 180F)), (double) (-Mth.cos(this.getYRot() * ((float) Math.PI / 180F))));
            }
        }
    }
