...
public class AtlatitanEntity extends SauropodBaseEntity implements KeybindUsingMount {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return ACCompat.temptable(Monster.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, 0.325D).add(Attributes.MAX_HEALTH, 400.0D).add(Attributes.KNOCKBACK_RESISTANCE, 1.0D).add(Attributes.ATTACK_DAMAGE, 8));
    }
    ...
    public void tick() {
        ...
        if (level().isClientSide()) {
            ...
            if(this.getRideableFor() > 0 && level().getRandom().nextInt(2) == 0 && !this.isDancing()){
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        ...
        this.setRideableFor(ACCompat.getInt(compound, "RideableTime"));
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!prev.consumesAction() && itemstack.is(ACItemRegistry.SERENE_SALAD.get()) && !this.isBaby()) {
            // The remainder is asked for once and reused — see ACCompat#craftingRemainder; from 26 it
            // is an ItemStackTemplate that has to be instantiated, so calling twice would build two.
            ItemStack remainder = ACCompat.craftingRemainder(itemstack);
            if (!remainder.isEmpty()) {
                ACCompat.spawnAtLocation(this, remainder);
            }
    ...
    public void positionRider(Entity passenger, MoveFunction moveFunction) {
        if (this.isPassengerOfSameVehicle(passenger) && passenger instanceof LivingEntity living && !this.touchingUnloadedChunk()) {
            ...
            moveFunction.accept(passenger, this.getX() + seatOffset.x, this.getY() + seatOffset.y + ACPlatform.passengersRidingOffset(this, passenger) - this.getLegSolverBodyOffset(), this.getZ() + seatOffset.z);
        } else {
    ...
    public AgeableMob getBreedOffspring(ServerLevel level, AgeableMob mob) {
        return ACCompat.createEntity(ACEntityRegistry.ATLATITAN.get(), level);
    }

    // 1.21.2 made LivingEntity#getScale() final — it is the age scale multiplied by the new scale
    // attribute now — and moved the overridable "how big is this one" hook to getAgeScale(). The
    // body is the same on both sides, and every call site keeps spelling getScale(), because that
    // is still the method that reads the finished value.
    //? if >=1.21.2
