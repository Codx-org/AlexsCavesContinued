...
public class DinosaurSpiritEntity extends Entity {
    ...
    *///?}
    ...
    private static final EntityDataAccessor<Optional<UUID>> PLAYER_ID = SynchedEntityData.defineId(DinosaurSpiritEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    *///?} else {
    ...
    private void tickTremorsaurus(Player player) {
        ...
        if (target != null) {
            ...
            if (inRange && this.abilityProgress >= 5) {
                if(!dealtDamage && ACCompat.hurt(target, ACDamageTypes.causeSpiritDinosaurDamage(level().registryAccess(), player), 3 + 2 * getEnchantmentLevel())){
                    dealtDamage = true;
                }
                this.setFading(true);
            }
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        if (tag.contains("UsingPlayerUUID")) {
            this.setPlayerUUID(ACCompat.getUUID(tag, "UsingPlayerUUID"));
        }
        this.setDinosaurTypeInt(ACCompat.getInt(tag, "DinosaurType"));
    }
