...
public class WaterBoltEntity extends Projectile {
    ...
    private static final EntityDataAccessor<Optional<UUID>> ARC_TOWARDS_ENTITY_UUID = SynchedEntityData.defineId(WaterBoltEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public void tick() {
        ...
        if (!level().isClientSide()) {
            ...
            for (int j = 0; j < 3 + random.nextInt(2); ++j) {
                this.level().addParticle(ACCompat.isInWaterOrBubble(this) || isBubbling() ? ParticleTypes.BUBBLE_COLUMN_UP : ParticleTypes.FALLING_WATER, this.getRandomX(0.5D), this.getRandomY(), this.getRandomZ(0.5D), 0, -0.1F, 0);
            }
        }
        if(wooshSoundTime <= 0){
            wooshSoundTime = 30 + level().getRandom().nextInt(30);
    ...
    *///?} else {
    ...
    private void damageMobs() {
        ...
        for (LivingEntity entity : this.level().getEntitiesOfClass(LivingEntity.class, bashBox)) {
            if (!isAlliedTo(entity) && !(entity instanceof DeepOneBaseEntity) && (owner == null || !entity.is(owner) && !entity.isAlliedTo(owner))) {
                lastHitMob = entity;
                if (ACCompat.hurt(entity, source, 3.0F) && this.isBubbling()) {
                    entity.addEffect(new MobEffectInstance(ACCompat.effect(ACEffectRegistry.BUBBLED.get()), 200));
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        this.setBubbling(ACCompat.getBoolean(tag, "Bubbling"));
