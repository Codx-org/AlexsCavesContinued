...
public class GumWormSegmentEntity extends Entity implements ICustomCollisions, KeybindUsingMount, PlayerRideableJumping, FlyingMount, CullingBoundsEntity, ACRiderSitEntity {

    private static final EntityDataAccessor<Optional<UUID>> HEAD_ENTITY_UUID = SynchedEntityData.defineId(GumWormSegmentEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    private static final EntityDataAccessor<Optional<UUID>> FRONT_ENTITY_UUID = SynchedEntityData.defineId(GumWormSegmentEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    private static final EntityDataAccessor<Optional<UUID>> BACK_ENTITY_UUID = SynchedEntityData.defineId(GumWormSegmentEntity.class, ACEntityDataRegistry.OPTIONAL_UUID.get());
    ...
    public void readAdditionalSaveData(CompoundTag compound) {
        if (ACCompat.hasUUID(compound, "HeadUUID")) {
            this.setHeadUUID(ACCompat.getUUID(compound, "HeadUUID"));
        }
        if (ACCompat.hasUUID(compound, "FrontUUID")) {
            this.setFrontEntityUUID(ACCompat.getUUID(compound, "FrontUUID"));
        }
        if (ACCompat.hasUUID(compound, "BackUUID")) {
            this.setBackEntityUUID(ACCompat.getUUID(compound, "BackUUID"));
        }
        this.setIndex(ACCompat.getInt(compound, "Index"));
    }

    public void addAdditionalSaveData(CompoundTag compound) {
        if (this.getHeadUUID() != null) {
            ACCompat.putUUID(compound, "HeadUUID", this.getHeadUUID());
        }
        if (this.getFrontEntityUUID() != null) {
            ACCompat.putUUID(compound, "FrontUUID", this.getFrontEntityUUID());
        }
        if (this.getBackEntityUUID() != null) {
            ACCompat.putUUID(compound, "BackUUID", this.getBackEntityUUID());
        }
    ...
    // See SauropodPartEntity#isInvulnerableTo — from 1.21.2 an Entity subclass no longer overrides
    // anything here, so the @Override goes and the super call becomes isInvulnerableToBase.
    //? if >=1.21.2 {
    /*public boolean isInvulnerableTo(net.minecraft.world.damagesource.DamageSource damageSource) {
        ...
        return this.isInvulnerableToBase(damageSource) || damageSource.is(DamageTypes.IN_WALL) || damageSource.is(DamageTypes.FALL);
    }
    *///?} else {
    @Override
    ...
    private void pushEntities() {
        if (this.level().isClientSide()) {
            ...
            if (!list.isEmpty()) {
                int i = ACCompat.gameRule(this.level(), GameRules.RULE_MAX_ENTITY_CRAMMING, 24);
    ...
    *///?} else {
    ...
    public AABB getBoundingBoxForCulling() {
        // Was super.getBoundingBoxForCulling(), which Entity declared as exactly this until 1.21.2
        // deleted it; see CullingBoundsEntity.
        return this.getBoundingBox().inflate(8.0F);
    }

    /**
     * The old {@code noCulling = true} the constructor set when the Entity Culling mod is present —
     * the segments are long enough that its per-entity occlusion test hides them behind their own
     * worm. 1.21.2 moved the flag to the renderer as {@code affectedByCulling}; see
     * {@link CullingBoundsEntity}.
     */
    @Override
    public boolean isAffectedByCulling() {
        return !ACLoadedMods.isEntityCullingLoaded();
    }
    ...
    public Vec3 getRiderPosition(Entity playerOwner) {
        float f = (float) (this.getBbHeight() + 0.25F + ACPlatform.myRidingOffset(playerOwner, this));
    ...
    // A segment is ridden, so vanilla would hand movement authority to the rider's client. Upstream
    // said no — the server owns a segment, because the worm it belongs to does — by overriding
    // isControlledByLocalInstance to isEffectiveAi, i.e. plain !isClientSide.
    //
    // 1.21.5 made that method final and split the decision it used to make into the two protected
    // halves it now consults. Answering false to both reconstructs exactly !isClientSide: on the
    // client isLocalClientAuthoritative decides and says no, on the server the answer is
    // !isClientAuthoritative and says yes. Overriding isEffectiveAi instead would be circular
    // there — it delegates to the final method.
    //? if >=1.21.5 {
    /*@Override
