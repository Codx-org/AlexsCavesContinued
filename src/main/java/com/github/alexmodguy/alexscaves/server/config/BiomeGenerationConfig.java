...
import com.github.alexmodguy.alexscaves.citadel.server.event.EventReplaceBiome;
...
public class BiomeGenerationConfig {
    ...
    private static <T> T getOrCreateConfigFile(File configDir, String configName, T defaults, Type type, Predicate<T> isInvalid) {
        ...
        try {
            ...
            // Gson hands back null for an empty file, and on a first launch that really happens: the
            // config-load and config-reload events both find the file missing, so one thread is
            // truncating it for its own FileUtils.write while the other reads. Treating null as
            // "invalid" rewrites the defaults instead of throwing an NPE out of isInvalid.test.
            if (found == null || isInvalid.test(found)) {
    ...
    private static File getConfigDirectory() {
        // CodxLib.configDir() rather than FMLPaths.CONFIGDIR — same directory, one call that
        // works on all three loaders.
        Path configPath = CodxLib.configDir();
